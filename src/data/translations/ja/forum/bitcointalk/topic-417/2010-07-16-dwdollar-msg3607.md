...
これはおそらく最良のアプローチではない（少し前にサトシと話した結果）。実際、後から追加されてVersion .3に入っているAPIの新機能はまだ適用していない。だが、実戦で検証されている。壊れていない（まだ）ものを直す必要があるか？ Bitcoinサーバを動かしているマシン上で別プロセスとして動く。唯一省略したのはwhileループのbreakハンドラだけだ。「list(something_inside)」を見たら、それはSQLObjectを使ったデータベースへのクエリだと思ってくれ。
