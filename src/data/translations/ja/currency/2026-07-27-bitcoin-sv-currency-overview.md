...
このチェーンは自らの設計を「Satoshi Vision」と呼び、掲げる目的は復元だとする。ブロックサイズの上限を外し、無効化されていたオペコードを復活させ、[クレイグ・ライト](/BitcoinArchive/ja/participants/craig-wright/)と nChain が言う「本来のビットコインプロトコル」を再び動かす、というものだ。だが、その復元が実際に何を引き継いだのか、実際に誰が握っているのか、実際にどう機能してきたのかは、別々の答えを持つ三つの問いである。
