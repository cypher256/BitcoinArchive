---
title: "Re: Bitcoin"
date: 2009-07-21T03:14:43Z
type: "correspondence"
source: "malmi-email-archive"
sourceUrl: "https://mmalmi.github.io/satoshi/"
sourceNote: "2024 年 2 月、COPA 対ライト裁判の証言の一環として GitHub で公開"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
  - name: "Martti Malmi"
    slug: "martti-malmi"
description: "サトシがハル・フィニーの初期の貢献に触れ、18ヶ月の開発後に休息が必要であることを認め、ビットコインのブートストラップ用アプリケーションのアイデアをマルミに求める。"
isSatoshi: true
tags:
  - "correspondence"
  - "early-contributor"
secondarySources:
  - name: "COPA v. Wright Trial Exhibits"
    url: "https://www.opencrypto.org/2024-02-22-witnesses-satoshi-correspondence/"
quotes:
  - id: "q1"
    person: "mmalmi@cc.hut.fi"
    personSlug: "martti-malmi"
    date: "2009-07-21T00:43:34Z"
    sourceEntryId: "correspondence/martti-malmi/2009-07-21-bitcoin-023"
relatedEntries:
  - "analysis/2008-08-20-satoshi-self-statements"
translationStatus: complete
---

<!-- speaker: Satoshi Nakamoto -->
馬鹿みたいに聞こえるかもしれないが、まだ Sourceforge のログインページを読み込めていないので、読むこともできていない。https://sourceforge.net/account/login.php

ハルは現在積極的には関わっていない。暗号学メーリングリストで設計を擁護する際にとても助けてくれたし、最初のリリース時の初期テストでも協力してくれた。彼は何年も前に Reusable Proof Of Work（RPOW）でこの松明を掲げていた。

私も今はあまり助けにならない。仕事がかなり忙しく、18ヶ月の開発の後で一息つく必要がある。

人々が使える何かがあると助かるのだが。ブートストラップのためのアプリケーションが必要だ。何かアイデアはあるか？

資金が必要なものを思いつけば頼れる寄付者がいるが、彼らは匿名を望んでおり、実際に何かするのが難しくなっている。

<!-- quote: q1 -->
<!-- tone-skip -->

> こんにちは、
>
> 1ヶ月ほど前に SF のビットコイン開発者フォーラムに投稿し、あなた、デイヴィッド、ハルに users.sourceforge.net のメールで通知を送りました。
> 数日前、なぜ誰も返信していないのか不思議に思い、SF のメールエイリアスが機能するか試してみたところ、少なくとも僕のアカウントでは機能していませんでした。
> このメッセージを他の方々に転送していただけますか？
>
> よろしくお願いいたします、
> sirius-m
>

<!-- /tone-skip -->
