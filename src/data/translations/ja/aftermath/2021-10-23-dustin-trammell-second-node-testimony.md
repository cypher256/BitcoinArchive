...
トランメルは、2009年1月9日の暗号技術メーリングリストでのサトシの発表後、ソフトウェアをダウンロードして実行した最初の瞬間を描写した：
...
彼が観察した単一の接続は、ほぼ確実にサトシ自身のノードだった——その時点でビットコインを実行している唯一の他のマシンだった。
...
この詳細は初期のネットワーク動態を理解する上で重要である。最初のユーザーの一人であったにもかかわらず、トランメルは最初の接続後数日間マイニングしていなかった。これは、ネットワークの初期のハッシュパワーが以前理解されていた以上にサトシの手に集中していたことを意味する。

**2番目のノードであるという推測：**

> 「ネットワーク上の2番目のノードだったかもしれないと思うが、それは当時のソフトウェアで見た動作に基づく完全な推測だ。」

トランメルは、これが確定的な証拠ではなく観察された動作（既知のIPへのブートストラップによる単一のピア接続）に基づいていることを慎重に述べた。しかし、v0.1リリースからわずか2日後の2009年1月11日にサトシに送ったメールで、クレジット0.00の「Generated」メッセージを文書化しており、その日までにソフトウェアを実行していたことが確認されている。
