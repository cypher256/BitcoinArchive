...
反論：本論点は他の長期サイファーパンク（アダム・バック、ウェイ・ダイ、ハル・フィニーら）にも広く当てはまる。候補集合を実質的に絞るが、サボを特定的に選び出すわけではない。§2.1 の表が示すとおり、この系譜の部品（プルーフ・オブ・ワーク、連結タイムスタンプ）自体が共通祖先からの継承（両列とも 🔄）であって、サボ起源ではない。
...
Bit Gold は概念的提案にとどまった。ビットコイン v0.1 の主要構成要素を欠く：難易度調整、最長チェーン合意形成規則、UTXO モデル、マイニング報酬による新規発行、2,100 万通貨上限、ECDSA に基づくトランザクション、稼働中の P2P ネットワーク。[ビットコイン設計系譜分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-design-lineage/)はこれらをビットコインの独自貢献として目録化しており、Bit Gold から継承できるものではない。§2.1 の表が示すとおり、Bit Gold が含んでいた部品でさえ大半は共通祖先由来（🔄）であってサボ独自ではない。サボの記録された仕事のパターンは概念的執筆であって、動作するコードベースを作り上げることではない — 2008 年 4 月の「10 年間 Bit Gold を実装していない」自認 (§3.1) はそのパターンの診断的特徴である。
