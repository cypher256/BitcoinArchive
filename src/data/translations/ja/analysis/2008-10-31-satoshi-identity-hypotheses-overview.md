...
12 人の名指し候補、サトシの公的記録から導かれる 7 つの次元。どの候補も 7 つすべてに合致しない。いずれも少なくとも 1 つの次元で破綻する。本エントリーは名指し候補を、プロファイル整合（§2）、文体計量による帰属（§3）、直接通信（§4）、開発環境（§5）という 4 つの独立した構造的レイヤで並列比較し、各プロフィールがどこで破綻するかを記す。各候補は、以下の[サトシ](/BitcoinArchive/ja/participants/satoshi-nakamoto/)に関する公的記録から導かれる輪郭に照らして比較する：
