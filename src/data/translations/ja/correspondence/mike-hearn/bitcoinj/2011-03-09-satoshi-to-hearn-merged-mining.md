...
素材となる「Bitcoinブロック」がBitcoinチェーンで実際に有効であったかどうかは問題ではない。ただし、有効であった可能性はある。BitDNSにとっては、その複雑なハッシュ計算を行うために必要な単なるソルトの集まりだ。マイナーがBitDNSのみをマイニングしていてBitcoinに関心がない場合、すべてゼロの空白Bitcoinブロック（ナンスを除く）を使用するだろう。
