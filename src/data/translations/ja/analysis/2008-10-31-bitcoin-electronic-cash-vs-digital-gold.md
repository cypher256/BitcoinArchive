...
ホワイトペーパーには、互いに引っ張り合うように見える二つの像がある。[タイトル](/BitcoinArchive/ja/entries/emails/cryptography/2008-10-31-bitcoin-whitepaper-final/)は電子キャッシュを掲げ、第 6 節は新しいコインを流通へ加える行為を金の採掘になぞらえる。この緊張は、二つの記述が同じ軸を指しているのかを確かめると見え方が変わる。その答えは、ホワイトペーパーの設計が最初に記録された購入の場面で実際にどう使われたか、そして希少性とスケーリングの制約が、その設計をデジタルゴールドへ押した経緯のなかに現れる。
...
ビットコインは、ある日突然「金」を選んで現金を捨てたわけではない。中央の発行者なしに価値を動かせる硬さが、同時に、値上がりする資産を手放して支払うことへのためらいを生んだ。ホワイトペーパーには今も二つの顔が一つの設計として残っている。どちらを人々が見るのかを決め続けているのは、動いているネットワークの方だ。
