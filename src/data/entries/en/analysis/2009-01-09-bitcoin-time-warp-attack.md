...
**The Great Consensus Cleanup.** [Antoine Poinsot](https://github.com/darosior) has championed a soft-fork proposal known as the **Great Consensus Cleanup** that bundles a fix for the time-warp bug with several other long-known protocol curiosities. The proposal would constrain the timestamp of the first block of each retarget window so that the off-by-one cannot be exploited regardless of hashrate share. As of 2026 the proposal remains in active community discussion: it has support from several Bitcoin Core developers but has not reached the level of operator consensus that triggered SegWit (BIP148) or Taproot (BIP341).
