...
日本語散文で節を橋渡しする補足に使うダッシュは **エムダッシュ (`—`、全角、前後に半角スペース)** のみに統一する。以下は字種の誤りであり、見つけ次第エムダッシュへ置き換える。

| 文字 | 用途 | 本文の橋渡し補足に使えるか |
|---|---|---|
| `—` エムダッシュ (U+2014) | 節を橋渡しする補足、言い換え、挿入句 | ✅ 正規 |
| `―` 水平バー (U+2015) | — | ❌ エムダッシュへ統一 |
| `─` 罫線 (U+2500、 box drawing) | 表・ボックス罫線専用の記号 | ❌ 散文中は常に誤り (罫線が紛れ込んだ字化けとして即修正) |
| `–` エンダッシュ (U+2013) | 日付・数値の範囲 (`2020–2024`) | 範囲用途のみ。橋渡し補足には使わない |
| `-` ハイフン (半角) | 複合語・ハイフネーション | 橋渡し補足には使わない |

一括置換は禁止 (§ 半角スペースの規約と同じ理由 — 罫線の中には ASCII アートの罫線として正当な用途があるため、各出現を文脈で判断する。例: `analysis/2009-01-03-genesis-block-hardcode-analysis.md` のコードブロック内 `├──`/`└──` はツリー構造の罫線であり対象外)。
