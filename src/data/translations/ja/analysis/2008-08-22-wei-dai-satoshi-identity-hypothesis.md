...
secondarySources:
  ...
  - name: "Gwern's Archive — ウェイ・ダイ／サトシ・ナカモト メール"
...
ビットコインの記録された年表に対する仮説の時系列:

```mermaid
timeline
    title ウェイ・ダイ vs サトシ - 記録された経緯
    1998 : b-money を サイファーパンクリストで 公開 (11月26日)
         : アダム・バックの b-money 批評 (12月6日)
    2007 : サトシ 開発期間開始 (本人の年表では 2007 年半ば)
    2008 : サトシが アダム・バックに接触、 バックが ウェイ・ダイへ紹介 (8月20-21日)
         : サトシが ウェイ・ダイに接触 - 「b-money のページは知らなかった」 (8月22日)
         : ホワイトペーパー公開、 b-money は 参考文献 [1] (10月31日)
    2009 : Bitcoin v0.1 が Crypto++ 5.5.2 SHA-256 をバンドル (1月)
         : サトシ → ウェイ・ダイ - 「b-money の目標のほぼ全てを達成」 (1月10日)
    2010 : v0.3.6 で Crypto++ 5.6.0 SSE2 最適化 SHA-256 を統合 (7月)
    2014 : ウェイ・ダイ AALWA 回顧 - サトシは 「以前から 活動していた 人物ではない」 (1月12日)
```
...
|---|---|---|
| v0.1 | 2009-01-09 | Crypto++ 5.5.2 SHA-256 を スタンドアロン (`sha.cpp`、`sha.h`) として バンドル |
...
ウェイ・ダイの候補空間での構造的位置は他の候補と異なる: 彼は候補集合の構成上 Group A（サトシがホワイトペーパーで明示的に引用）に属するが、引用経路を確立する同じ一次資料メールが、ウェイ・ダイが第三者としてビットコインをサトシから受け取ったこと（彼が起源ではないこと）も同時に確立する。
