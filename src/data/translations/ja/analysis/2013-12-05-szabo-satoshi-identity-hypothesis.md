...
| ビットコインの構成要素 | Bit Gold (1998 / 2005) | ビットコイン (2009) | 出自（発明者） |
|---|---|---|---|
| プルーフ・オブ・ワーク | ✅ | ✅ | Hashcash（アダム・バック 1997）／サボではない |
| 連鎖 PoW（解かれたパズルの連結） | ✅ — 概念のみ | ✅ | ハーバー・ストルネッタ／サボが PoW に適用 |
| 分散タイムスタンピング | ✅ — 概念のみ | ✅ | ハーバー・ストルネッタ（1991）／サボではない |
| 反信頼の貨幣枠組み | ✅ | ✅ | ハイエク／サイファーパンク圏／サボではない |
| 最長チェーン合意形成規則 | ❌ | ✅ | サトシ — 新規 |
| UTXO モデル | ❌ | ✅ | サトシ — 新規 |
| 難易度調整 | ❌ | ✅ | サトシ — 新規 |
| 2,100 万通貨上限 | ❌ | ✅ | サトシ — 新規 |
| マイニング報酬による新規発行 | ❌ | ✅ | サトシ — 新規 |
| ECDSA に基づくトランザクション | ❌ | ✅ | サトシ — 新規（ECDSA は標準） |
| 稼働中の P2P ネットワーク | ❌ | ✅ | サトシ — 新規 |
| 動作する実装 | ❌ — 構築されなかった概念 | ✅ — 19,901 行の C++ | サトシ — 新規 |

概念上の重なり（表の上段）はフォレンジック整合論が依拠する部分 — だが出自列がその重みを限定する。共有された行は両設計が共通祖先から引いた部品であり、ビットコインが Bit Gold から取ったものではない。プルーフ・オブ・ワークは Hashcash、連結タイムスタンプと連鎖順序はハーバー・ストルネッタ、反信頼の枠組みはハイエクとサイファーパンク圏に由来し、いずれも[ビットコイン設計系譜](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-design-lineage/)で「発明ではなく再利用」として目録化されている。サボ固有なのは合成 — プルーフ・オブ・ワークを連鎖して希少で所有が追跡される単位にする「偽造不能なコスト」 — であって、これは Bit Gold が実装しなかった概念であり、ビットコインは下段の合意機構によって別途実現した（すべてサトシ独自の貢献）。
...
反論：本論点は他の長期サイファーパンク（アダム・バック、ウェイ・ダイ、ハル・フィニーら）にも広く当てはまる。候補集合を実質的に絞るが、サボを特定的に選び出すわけではない。§2.1 の出自列が示すとおり、この系譜の部品（プルーフ・オブ・ワーク、連結タイムスタンプ）自体が共通祖先からの継承であって、サボ起源ではない。
...
Bit Gold は概念的提案にとどまった。ビットコイン v0.1 の主要構成要素を欠く：難易度調整、最長チェーン合意形成規則、UTXO モデル、マイニング報酬による新規発行、2,100 万通貨上限、ECDSA に基づくトランザクション、稼働中の P2P ネットワーク。[ビットコイン設計系譜分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-design-lineage/)はこれらをビットコインの独自貢献として目録化しており、Bit Gold から継承できるものではない。§2.1 の出自列が示すとおり、Bit Gold が含んでいた部品でさえ大半は共通祖先由来であってサボ独自ではない。サボの記録された仕事のパターンは概念的執筆であって、動作するコードベースを作り上げることではない — 2008 年 4 月の「10 年間 Bit Gold を実装していない」自認 (§3.1) はそのパターンの診断的特徴である。
