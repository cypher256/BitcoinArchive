...
ビットコインが確率的ファイナリティを選んだのは、システムを検閲耐性にする決定的な特性である、開放的で許可不要の参加者集合を可能にするためである。閉鎖的メンバーシップの BFT システムは即時ファイナリティを提供できるが、参加者が誰かを知ることが前提となり、ビットコインが排除するように設計された信頼の前提を、再び持ち込むことになる。
