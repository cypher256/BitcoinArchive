...
この 3 件は、 サトシが OSS 慣習を「知らなかった」 のではなく「知った上で意図的に取り込みを抑えた」 ことを示す。 ただし、 これらの発言はいずれも**コミュニティからの問い・要請に駆動された**ものであり、 サトシ自身が能動的に持ち出した OSS 慣習話題ではない。 Linux と同じパターン: Phase 1 沈黙 → Phase 2 でコミュニティに揉まれて受動的に表面化、 そして取り込みには消極的な応答を返す。
