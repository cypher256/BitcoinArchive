...
16 年間の公開された追跡試行には、数百件のフォーラム投稿、複数のメーリングリスト・アーカイブ、メール群、ソースコード、1000 ページ超のジャーナリズムが含まれるが、すべて同じ問いに向けられてきた。そして精査に耐える特定は一つもない。本エントリは、(a) サトシ・ナカモトを特定しようとする公開された試行の反復、(b) サトシが残した発信材料の総量、(c) 続く不明状態という、公開記録上の 3 つの観察可能な事実の間の関係を構造的な非対称性として読む。推論的な読み方は §6（限界）に限定する。

サトシが誰かについては[身元仮説総覧](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identity-hypotheses-overview/)に、サトシがどう匿名を保ったかについては[匿名性のアーキテクチャ](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-anonymity-architecture/)に、それぞれ別の読み解きがある。ここで扱うのはその両方を裏返した問いだ。これだけの材料と試行が積み重なってなお、なぜ不明状態が続くのか。
...
非対称性は、問いに費やされた可視の作業と、可視の結果の間にある。本エントリの枠組みでは、これは「サトシ vs 世界の敵対者」の非対称性ではない。その枠組みは公開外の追跡規模についての推論を要求するため、本エントリは採用しない。
