...
リーはライトコインを公的に **「ビットコインの金に対する銀」** として位置づけた — この枠付けは 2010 年代後半から 2020 年代に至るまで、チェーンのマーケティング文言として残り続けた。この比喩がリーの位置づけの意図を示している。ライトコインはビットコインの代替ではなく、日常取引向けの補完的・高速・低手数料のチェーンであり、ビットコインは価値保存用途のために残す、というものだった。

Scrypt の PoW 選択は、リーが期待した長期的な ASIC 耐性を生まなかった。Scrypt 対応 ASIC (代表的には KnCMiner、後の Bitmain Antminer L シリーズ) が 2014 年以降出荷され、ライトコインの採掘は 2015 ~ 2016 年までにビットコインと同程度に ASIC 支配下に入った。「ASIC 耐性」 の枠付けは、明示的に取り下げられることなく姿を消した。チェーンは Scrypt のまま動作し続けた。
