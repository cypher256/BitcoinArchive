...
過去事例: 9 BIP JA エントリーのうち 7 件が `translationStatus: complete` と表示しながら本文散文が 95-100% 未訳という嘘表示を起こした (2026-05-17 修正)。視覚的に弱い傾斜文字一段落の編者注では指摘が放置されたため、ブロック引用 + 太字リード形式に格上げした経緯がある。
...
BitcoinTalk 由来の引用は元から一貫してこの扱いだった ( 生の `[Quote from: NAME]` は chip 化と同時に除去、削除済みソースで chip を張れない場合のみ `<!-- audit:quote-skip -->` と共に残す )。書簡・メーリングリストの `NAME wrote:` 系だけがこの扱いから漏れ、chip 追加後も生テキストが本文に残ったまま出荷され続けていた。2026-07-13 に 21 ファイル ( 一次資料 EN + JA ) を手動で確認・除去して解消。
...
| `ship the codebase / ship a fix / ship v0.x / an idea gets shipped` (ソフト・コード文脈の `ship`) | `コードベースを出荷した` / `修正を出荷した` / `v0.x を出荷した` / `アイデアが出荷される` | `コードベースを公開した・世に出した` / `修正を公開した` / `v0.x をリリースした・公開した` / `アイデアが形になる・実装される` (2026-06-27 アーカイブ横断 19 ファイル約 70 箇所で是正、コミット `225d1d938`) |
...
**過去の違反例・本節確立の契機 (2026-08-13 ユーザー指摘で発覚):** 2026-03-20 の口調統一の一括修正コミット (`03bee0744` 等) が、既に正しく訳されていた「よろしくお願いします、」(サトシ→ウェイ・ダイのメール結び、原文 `Thanks,`) を、だ/である 適合のためだけに機械的に「よろしく頼む、」へ置換した。単語の口調適合だけを見て意味を確認しなかったため、依頼調への意味変化と (カンマ+即改行の構造と相まって) 呼びかけ調の誤読リスクを同時に発生させた。同種の機械置換で Constance Wells のメール(`correspondence/martti-malmi/2011-01-25-bitcoin-question-255.md` および、同じ文面が引用として再掲される `256.md`)でも同じ「よろしく頼む」が生じており、そちらは口調とは別に「Thank you.」(感謝の表明) を「よろしく頼む」(依頼) と訳す意味の逆転も伴っていた。教訓: だ/である 変換のような機械的な口調修正パスは、変換後の一語一句について意味を再確認してから確定する — 口調適合だけを見て意味を見ないと本節のような事故が起きる。
...
### audit の合格判定前チェック (2026-06-30 判定精度是正)

第三者 audit で「合格」 と判定した箇所が後でユーザー指摘により polish 戻しになる事故が複数回あった (直訳調・カタカナ過多・英語役職名残置を見逃していた)。合格判定を出す前に以下を確認する:
...
2026-05-31: `analysis/2026-05-23-how-bitcoin-works-visual-glossary` (JA) で複数の直訳調が放置されていた。 コミット `c2dfe469` で「ここまでが『型』 だ」 → 「ここまでが基本的な仕組みだ」 等 4 箇所を修正。 当該記事は本アーカイブの最初の読者導線にあり、 直訳調が読者の離脱を招く位置にあった。 本節はこの事故の再発防止として追加。

## 10. 語感の逸脱 — 文法的に正しくても事実と食い違う語

翻訳として意味が通り、文法的にも正しい語が、日本語での含意のレベルで記録された事実と食い違うことがある。§ 9 が英文の構造の写し取りを扱うのに対し、本節は一語の含意のずれを扱う。

典型例: 英語の `worked with X` を「X と働いた」と訳すと、日本語の「働く」は文脈によって雇用・職業的労働を想起させる。実際の関係が無償の協力や趣味の共同開発である場合、この訳は記録にない雇用関係を読者に含意しかねない。「協力した」「共に開発した」など、記録された関係の性質に合う動詞を選ぶ (2026-08-14〜15 に論説系エントリーで複数箇所を是正)。
...
実例 (2026-08-23、本節の由来): 分析ページの見出し「動いたもの：金」を、読者がまず「金 (ゴールド)」と読み、後続の文脈から初めて「資金」の意味だと読み直した。日本の金融報道 (日経等) を確認したところ、money の意味で裸の「金」を使う実例は見当たらず、常に「資金」「マネー」に置き換えている。gold の意味では、金価格・金 ETF・金相場のように後続の名詞が読みを固定する複合語の中で裸の「金」のままにする一方、ビットコインとの直接比較のような単独の名詞として置く場面では「ゴールド」を使う。
