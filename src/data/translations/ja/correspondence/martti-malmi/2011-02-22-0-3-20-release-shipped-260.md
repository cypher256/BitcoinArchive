...
*[補足：サトシからギャビン・アンドレセンとマルッティ・マルミ宛のメール（To にギャビンとマルッティの両方）。「0.3.20 release : shipped」 スレッドへの返信。本文の下方にある「ギャビン宛」「マルッティ宛」 はサトシ自身が二つの PGP 暗号化ブロックに付した見出し（各受信者の公開鍵で別々に暗号化）であり、 メールの別宛先ではない。]*
