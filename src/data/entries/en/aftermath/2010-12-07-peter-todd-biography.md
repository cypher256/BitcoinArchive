...
Todd became an active Bitcoin Core contributor, focusing on protocol-level security, transaction policy, and network resilience. He was a vocal participant in technical debates about Bitcoin's direction.

**Replace-by-Fee (RBF):**
Todd is best known for championing Replace-by-Fee (RBF), which allows unconfirmed transactions to be replaced by new versions with higher fees. The concept was formalized in BIP 125, co-authored by David A. Harding and Peter Todd.

**OpenTimestamps:**
Todd created OpenTimestamps, an open-source project that uses the Bitcoin blockchain to create tamper-proof timestamps, allowing anyone to prove that a document existed at a particular point in time.
