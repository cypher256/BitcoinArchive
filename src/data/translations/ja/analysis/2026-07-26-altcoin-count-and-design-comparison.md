...
sourceNote: "末尾の節で引く各社の数はいずれも 2026 年 7 月 26 日に読み取った現在値である。日々変わり、しかも互いに置き換えられない。数える対象が社ごとに違うからで、それ自体がその節の主題である。それ以外の記述はすべて、各企画の文書または取材そのものを典拠としている。"
...
participants:
  ...
  - name: "Jackson Palmer"
    slug: "jackson-palmer"
description: "その通貨の設計は、値が上がることを意図しているのか、自ら調整するのか、動かないことを目指すのか。12 チェーンを、その規則に書かれた答えで分類する。"
isSatoshi: false
tags:
  - "analysis"
  - "altcoin"
...
これは値が上がることを意図した設計なのか、自ら調整する設計なのか、動かないことを目指した設計なのか、それとも 1 単位の価値をそもそも問題にしていない設計なのか。どのチェーンも、誰かが買うより何年も前に、自分の規則の中でこの問いに答えている。その答えが、通貨にとっての思想にもっとも近いものだ。このページは 12 のチェーンをその答えで分類し、それぞれの立場が読み手の見る二つの数 ─ 1 単位の価格と時価総額 ─ に何をするのかを算術で確かめ、作った人々が何を作っていると述べたかを記録する。

アーカイブの他の場所に本拠がある主題は、繰り返さずにそちらへ送る。[各設計がなぜその発行規則を選んだか](/BitcoinArchive/ja/entries/analysis/2008-10-31-fixed-supply-vs-adjustable-money/)は 1998 年から続く議論であり、[そもそもビットコインになぜ価値があるのか](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-digital-gold-structural-features/)は構造の議論であり、[その希少性がなぜ現金としての用途を削ったのか](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-electronic-cash-vs-digital-gold/)は、後発のすべての設計が答えねばならなかった緊張である。「そもそも何種類あるのか」は最後に置いた。この一群について分かることの中で、もっとも実りの薄い問いだからだ。

## 1. その通貨は自分を何だと思っているか

時価総額とは、1 単位の価格に、流通している単位数を掛けたものである。チェーンの発行規則が決めるのは、この三つの項のうち一つ ─ 単位数だけだ。しかもそれを何年も前に、コードの中で、買い手の行動とは無関係に決める。
...
その立場は五つに分かれ、それぞれに、読み手が実際に見る二つの数への帰結が定まっている。各企画が**なぜ**その立場を採ったのか ─ どの失敗を避けようとしたのか ─ は[固定供給と自動調整の比較](/BitcoinArchive/ja/entries/analysis/2008-10-31-fixed-supply-vs-adjustable-money/)の主題で、そちらは 1998 年の b-money からイーサリアムのマージまでその議論をたどっている。ここに置くのは分類とその算術である。

| その通貨が自分を何だと考えているか | 規則がそれをどう言うか | 該当 | 時価総額を止めたときの 1 単位の価格 | 時価総額への作用 |
|---|---|---|---|---|
| **希少 ─ 薄められてはならない** | 上限を置き、発行はゼロへ減衰する | ビットコイン、ライトコイン、BCH、BSV、カルダノ | 下がる。ただし下がる速さは期ごとに半分になり、上限で止まる | なし |
| **希少、かつ一度に全量を発行** | 公開時に供給の全量を生成 | XRP | 発行計画による希釈はない。走らせる計画がもう残っていない | なし |
| **自動調整 ─ 供給はネットワークに従う** | 発行に手数料の焼却を足し、供給は縮みうる | イーサリアム | 設計では定まらない。向きは使用量の出力であって、計画の出力ではない | なし |
| **公開された曲線に沿った自動調整** | 逓減する率、またはゼロにならない恒久的な末尾発行 | ソラナ、モネロ | 永久に下がる。ただし下がる速さは縮み続ける | なし |
| **持つ資産ではなく、使う通貨** | 一定の量、または一定の割合を永久に発行 | ドージコイン、ポルカドット | 永久に下がる。ドージコインは母数が増えるにつれ率が落ち、ポルカドットは落ちない | なし |
| **釘付け ─ 動いてはならない** | 1 ドルを保つよう供給が伸縮する | USDT、USDC | 作りからして固定されている | なし |
...
**仕組みは 1 単位の価格の向きを決める** ─ ただし残差としてのみ、そして時価総額を止めた場合にのみ。単位が増えれば一つあたりは薄まり、規則がどれだけ増えるかを言う。供給設計と価格を結ぶ因果は、それで全部である。

**どの供給設計も、時価総額を上げも下げもしない。** 半減は買い手を増やさない。焼却も買い手を増やさない。上限も買い手を増やさない。いずれも、与えられた時価総額を単位のあいだでどう割るかを変えるだけで、割られる数そのものは変えない。その数が上がるかどうかは需要の問題であり、需要は表のどの仕組みの外側にもある。

ドル連動通貨がその独立を証明している。逆向きに実験しているからだ。USDC の時価総額は絶えず動く ─ 預けられたドルの量そのものだから ─ 一方で 1 単位の価格は、設計上まったく動かない。時価総額は変わり、1 単位の価格は変わらなかった。仕組みが両者を結んでいるなら、これは起こりえない。

ここから二つの帰結が出る。どちらも、よくある読み方と逆を向く。

**「永久にインフレ」は「一定率で希釈され続ける」を意味しない。** ドージコインは毎年同じ 52.56 億単位を、毎年増える母数に対して発行する。だから規則が一度も変わらないまま、新規発行が供給に占める割合は毎年落ちていく。希釈率が一定に保たれるのは、ポルカドットのように割合を目標に置いた設計だけである。
...
以上のどれも、1 単位の価格を比較の材料にはしない。ビットコインは 2,100 万単位で頭打ちになり、XRP は公開時に 1,000 億単位を生成し、ドージコインは毎年 52.56 億単位を足す。1 兆単位が発行済みで 0.2 ドルの通貨は、100 万単位で 200 ドルの通貨より大きな資産である。1 単位あたりの数字は「安いから伸びしろがある」という直観を誘うが、算術はそれを支持しない。割る側の単位数は公開時に選ばれたもので、需要については何も語らないからだ。逆転にもっとも近づいた場面 ─ 2017 年 6 月にイーサリアムがビットコインの時価総額の約 85% に達したこと ─ は、日付つきの記録とともに[フォークと隣接通貨の系譜](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-fork-and-altcoin-genealogy/)に置いてある。

### そもそも「値が上がる」ことを意図した設計はあるのか

各企画がなぜその発行規則を選んだのか ─ どの失敗を避けたのか ─ はこのページの主題ではない。[固定供給と自動調整の比較](/BitcoinArchive/ja/entries/analysis/2008-10-31-fixed-supply-vs-adjustable-money/)が、上限が裁量を恐れたことからモネロが安全性予算の枯渇を恐れたことまで、その賭けを一つずつ並べている。ここで問うのはもっと狭い。各設計は 1 単位の**価値**がどうなることを望むと述べているか。答えは三つに分かれる。薄められないようにする（上限のあるチェーン群）、そもそも動かさない（ドル連動通貨）、価値は自分の担当ではないとする（性能重視のチェーン群と、ドージコイン）。
...
つまり値上がりの議論は、仕様書ではなく保有者の側から出ている。それはこの議論への反論ではない。希少で裁量の効かない資産は、設計者がそう言ったかどうかとは無関係に持つに値しうる。その構造版の議論は[デジタルゴールドの読み](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-digital-gold-structural-features/)にあり、[サトシ自身の生産費の議論](/BitcoinArchive/ja/entries/analysis/2010-02-21-bitcoin-price-vs-production-cost/)は、この歴史の中で設計者が価格について直接に推論している唯一の場所である。ここで言いたいのは、二つの主張を分けて読むべきだということだ。企画の側がそう分けているのだから。

例外は §7 で、そこでは問いが裏返る。供給が公開前に発行者へ割り当てられ、手数料収入が価格の向きに関係なく発行者へ入る設計は、価値がどこへ行くかについて極めて明確な見解を持っている。ただしそれは、1 単位の価格についての見解ではない。

### 板挟みと、その下にある問い

二つの半分を合わせると、この分類は罠になる。

薄まらないよう単位を守る設計は、その単位を持つに値するものにする。そして持つに値するものは使われない。これが[電子キャッシュの読み](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-electronic-cash-vs-digital-gold/)が、ピザから決済層まで、ビットコイン自身の歴史を通してたどった機構である。だが逆も成り立つ。そしてそちらは、あの記録が扱っていない半分だ。単位が使えるままであるよう発行し続ける設計は、それを溜め込む理由を取り除いた設計でもある。ドージコインは安く、速く、動く。[公開の記録](/BitcoinArchive/ja/entries/aftermath/2013-12-06-dogecoin-launch/)は、その持続的な意義は技術ではなく社会的なものだと結論している。位置を保ったのは共同体と知名度によるのであって、貨幣としての設計が良い貨幣にしたからではない。ドル連動通貨がもっとも明快な事例で、膨大な量が保有されていながら、作りからして、値上がりを期待して持つ者はいない。
...
§3 の「公平な立ち上がり」の列が道徳の点数表ではなく構造の点数表なのは、そのためだ。公開前の配分を持つ創設者は、「この設計は正しいか」という問いと「この設計は自分に払うか」という問いを切り離せない。後者の答えが前者に依存しないからである。この記録の中で、その絡まりを意図的に断ち切った創設者はちょうど一人いる。[チャーリー・リー](/BitcoinArchive/ja/participants/charlie-lee/)は 2017 年 12 月に自身のライトコインをすべて売却または寄付し、理由を明言した。保有しながら発言するのは利益相反である、自分は価格に対して影響力を持ちすぎている、と。その行為が稀であることが、ここでの発見であって、行為そのものではない。
...
むしろ、どんな比較表を読むときも ─ このページを含めて ─ 二つを分けて読むべき理由になる。§1 の思想は各チェーンが何であろうとして作られたかを説明する。なぜ誰かがそれを買ったかは説明しない。
...
## 5. 各チェーンがなぜその設計を選んだか
§2 の設計にはそれぞれ、作った当人が公にした理由がある。簡潔にまとめる。詳細はそれぞれの記録にある。
...
このアーカイブはこれらの出来事を記録し、その背後の意図を性格づけない。言えるのは構造の話で、それがこの節を置いた理由でもある。§3 が試す性質は、この種のコインでは作りからして全部欠けている。供給は公開前に発行者へ割り当てられ、発行者は名前のある主体であり、公開はその主体自身が告知し、手数料収入は価格の向きに関係なくその主体へ入る。ビットコインが持っていた立ち上がりの条件のちょうど裏返しであり、しかもいまや、その過程は一日に数万回動かせるほど安い。

## 8. アルトコインは何種類あるか（そして、なぜそれが一番実りの薄い問いか）
...
そのほとんどは続かない。ここでも測定は定義に依存する。pump.fun のトークンについて、CoinGecko Research は 68.67% が公開当日に最後の取引を記録したとし、学術的な生存分析は実際の市場に到達する率を 0.198% としている。2017 年前後の ICO については、Satis Group の評価が約 78% を詐欺と判定し、取引所で取引されたのは約 15% だった。Boston College の調査では、資金調達も上場もしなかった企画の 83% が不活発になったのに対し、両方を達成した企画では 16% だった。消滅した通貨を数える一覧どうしは桁で食い違う。それぞれが自前の定義を使っているからである。
