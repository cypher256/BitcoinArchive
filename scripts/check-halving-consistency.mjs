 ...
 *
 * The in-body supply-curve embed's runtime (src/scripts/chart-embed-runtime.ts,
 * imported by src/components/ChartEmbedRuntime.astro) is a module script that
 * cannot import JSON server-side data at its own scope, so it keeps a
 * self-contained copy of the named halving schedule (date + reward) and feeds
...
const EMBED = join(root, 'src/scripts/chart-embed-runtime.ts');
...
if (!block) {
  console.error('check:halving-consistency: could not locate the embed `halvings: [...]` array in src/scripts/chart-embed-runtime.ts');
  process.exit(1);
}
...
if (problems.length) {
  ...
  console.error('Fix: edit the `halvings: [...]` array in src/scripts/chart-embed-runtime.ts to match src/data/btc-chart/halvings.json.');
  process.exit(1);
}
