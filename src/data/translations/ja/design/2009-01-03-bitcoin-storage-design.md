...
初期ブロックダウンロード（IBD）— ジェネシスブロックから現在のチップまですべてのブロックを検証すること — は、ハードウェアとネットワーク速度に応じて数時間から 1 日以上かかる。**assumeUTXO**（v24.0 以降段階的に導入、v27 以降で完全サポート）は代替のブートストラップパスを提供する: ノードは事前計算された UTXO スナップショットを読み込み、スナップショット高から即座に新規ブロックの検証を開始し、バックグラウンドで完全な過去のチェーンを検証する。
...
| **assumeUTXO** | 利用不可 | バックグラウンド検証付きスナップショットベースのブートストラップ（v27 以降） |
