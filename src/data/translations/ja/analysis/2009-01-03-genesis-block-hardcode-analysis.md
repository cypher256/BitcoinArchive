...
- **解釈 B**：同じ余裕は重み付け的な刻印と読める — 非人格化された設計の中で、*The Times* 見出し（編集的コンテンツ）と組になる可視の「目標値下余裕ハッシュ」を、設計が依然として帯びる個別的要素の 2 つとして残す
...
5. その選択が実装上の便宜（解釈 A）だったのか脱帰属設計（解釈 B）だったのかは、ソースコード単体では判別できない。行動証拠 — 必須でなくより大きい実装の選択、サトシの他の非人格化実践との整合、coinbase アドレスの共有定数化、*The Times* 見出しを唯一の個人的要素とする構造、§6 と同じオッカムの剃刀 — を加味すると、解釈 B が非対称に有利となる
6. Block 0 には二次的な観察がある：ジェネシスハッシュは難易度 1 の目標値を数値的に大きく下回り、v0.1 の Block 0 コンセンサスは等価性のみで PoW 数値比較を呼ばない。これを「意図的な重み付け」と読めば解釈 B のパターンと整合するが、「外部者の PoW 形式検証が境界すれすれで通るのを避けるための保守的なエンジニアリング選択」と読めば解釈 A と整合する。機構 B 単独より診断度は低く、*The Times* 見出しと組で読む時の方が単独より目立つ
