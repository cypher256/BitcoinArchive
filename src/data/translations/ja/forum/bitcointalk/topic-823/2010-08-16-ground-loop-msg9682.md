...
>
> そうだ、修正されたチェーンに取り込まれ直す。トランザクションは消えず、両側で引き続き見えるが、コンファメーション数は 0 に戻ってまたカウントが上がり始める。
>
> 50 BTC が消えるのは、ブロック 74638 以降に不正チェーンでブロックを生成した場合だけだ。不正チェーンで生成したブロックは、いずれにしてもまだ成熟していないはずだ。
