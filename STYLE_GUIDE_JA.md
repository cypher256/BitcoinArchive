...
統一先の選び方: ずれている掲載箇所のいずれかが一次資料エントリー (その発言を逐語保存する primary-source 型) にある場合は、一次資料側の訳を正典とし、他方をそちらへ揃える (一次資料側の訳自体に誤り・欠落が確認された場合は、原文と照合して先に一次資料側を正してから揃える)。同一エントリー内で引用ブロックと本文中の再引用 (「…」 による部分引用) がずれている場合も同じ — 再引用は、そのページの引用ブロック、または一次資料の正典訳に一致させる (部分引用は段落単位の機械検出に掛からないことがあるため、執筆時に目視で揃える)。
...
典型例: 英語の `worked with X` を「X と働いた」と訳すと、日本語の「働く」が持つ雇用・有償労働の含意が付いてくる。実際の関係が無償の協力や趣味の共同開発である場合、この訳は記録にない雇用関係を読者に含意してしまう。「協力した」「共に開発した」など、記録された関係の性質に合う動詞を選ぶ (2026-08-14〜15 に論説系エントリーで複数箇所を是正)。
