 ...
 *
 * The in-body supply-curve embed (public/scripts/chart-embeds.js) is a
 * standalone browser script that cannot import JSON at build time, so it keeps
 * a self-contained copy of the named halving schedule (date + reward) and feeds
 * it to the shared drawer (public/scripts/supply-curve.js). Everything else —
...
const EMBED = join(root, 'public/scripts/chart-embeds.js');
...
if (!block) {
  console.error('check:halving-consistency: could not locate the embed `halvings: [...]` array in public/scripts/chart-embeds.js');
  process.exit(1);
}
...
if (problems.length) {
  ...
  console.error('Fix: edit the `halvings: [...]` array in public/scripts/chart-embeds.js to match src/data/btc-chart/halvings.json.');
  process.exit(1);
}
