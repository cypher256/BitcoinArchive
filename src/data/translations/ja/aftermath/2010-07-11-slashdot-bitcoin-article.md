---
title: "Slashdotがビットコイン記事を掲載 — 初の大手テックメディア報道"
date: 2010-07-11T00:00:00Z
type: "article"
source: "slashdot"
sourceUrl: "https://news.slashdot.org/story/10/07/11/1747245/bitcoin-releases-version-03"
author: "Slashdot"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "当時最も影響力のあるテクノロジーニュースサイトの一つであるSlashdotが、Bitcoin v0.3に関する記事を掲載した。「Slashdot効果」によりダウンロード数と新規ユーザーが急増し、ビットコインにとって初の重要なテックメディア露出となった。"
isSatoshi: false
tags:
  - "slashdot"
  - "media"
  - "bitcoin-v0.3"
  - "growth"
  - "historic"
secondarySources:
  - name: "Bitcoin Magazine — Bitcoin Gets Slashdotted"
    url: "https://bitcoinmagazine.com/technical/bitcoin-gets-slashdotted-creation-mt-gox-1432938208"
  - name: "Blockworks — Bitcoin's journey to the mainstream started 15 years ago"
    url: "https://blockworks.co/news/satoshi-slashdot-bitcoin-evolution"
relatedEntries:
  - sourceforge/2010-07-06-bitcoin-v03-released
  - emails/bitcoin-list/2010-07-06-bitcoin-0-3-released
  - aftermath/2010-07-15-jeff-garzik-biography
  - forum/bitcointalk/topic-1347/2010-10-04-jgarzik-msg15215
  - aftermath/2010-08-15-value-overflow-incident
translationStatus: complete
---

2010年7月11日、Slashdotが「Bitcoin Releases Version 0.3」というタイトルの記事を掲載した。大手テクノロジーニュースプラットフォームでのビットコイン初の重要な報道である。

「Slashdot効果」は即座に劇的だった。ビットコインソフトウェアのダウンロード数は2010年6月の約3,000から7月には20,000以上に急増した。新規ユーザーがネットワークに殺到し、ビットコインの価格はその後数日間で急騰した。

これはマイルストーンであると同時に課題でもあった。ユーザーの急激な流入はネットワークの処理能力を試し、それまで理論的だったスケーラビリティの限界を露呈させた。サトシと他の開発者たちは帯域幅の制約、メモリー管理、トランザクション処理のボトルネックに対処し、ソフトウェアの最適化に取り組んだ。

Slashdot記事は望まざる注目も集めた。好奇心旺盛な技術者と共に、投機家、批判者、そして — やがて判明するように — 攻撃者もやってきた。1か月後、ネットワークは最も深刻な危機に直面することになる：[2010年8月15日のvalue overflow事件](/BitcoinArchive/ja/entries/aftermath/2010-08-15-value-overflow-incident/)である。
