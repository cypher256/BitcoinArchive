...
**固有名の内部では分割しない**。空白挿入位置は固有名 (人名・チェーン名・引用タイトル等) の**外側**を選ぶ。固有名内に空白を入れると視覚的な断絶 (例: タイトル「ビットコイン実験の終結」 → `「ビットコイン 実験の終結」` で `ン 実」` が浮く) を生む。固有名そのものが span を 12 全角超に押し上げている場合は、ASCII 略号 (`BCH`、`BSV` 等) で span を断ち切るか、言い換えで長い span 自体を回避する:

```text
✗  「ビットコイン 実験の終結」     (タイトル内分割)
✗  「オリジナル プロトコル」        (引用句内分割)
✗  ビットコインキャッシュ ハードフォーク (固有名と一般語の境界、固有名直後で切れて視覚的に不自然)

✓  ハーンが ビットコイン離脱を表明      (タイトルを平叙に言い換え)
✓  BCH 分岐                              (略号 + 一般語、自然な境界)
