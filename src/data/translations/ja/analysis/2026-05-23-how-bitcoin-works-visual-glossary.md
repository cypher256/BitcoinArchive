...
description: "完全初心者向けのビットコイン視覚ガイド。 通貨・コンピューター・暗号学の事前知識は一切不要。 8 章にわたって 30 個の用語を、 章ごとの図と一緒に並べる: UTXO としてのコイン、 入力と出力で構成されるトランザクション、 ハッシュで連なるブロック、 抽選としてのマイニング、 待合室としてのメモリープール、 最長チェーンへの同意としての合意。"
isSatoshi: false
homeOrder: 11
tags:
  - "beginner-guide"
...
このエントリーは、 ビットコインのことも、 暗号学のことも、 コンピューター同士がどうデータをやり取りするかも、 何ひとつ知らない読者を前提にしている。 読み終える頃には、 Archive の残りを専門用語に詰まらずに読み進められるはずだ。 主要な用語はすべて、 初出のところで一行の定義と、 仕組み上どこに位置するかを示す図を添える。
...
| 用語 | 一行の意味 | 章 |
|---|---|---|
| ノード | ビットコインソフトウェアを動かしているコンピューター | [1](#1-ビットコインって何) |
| ピアツーピア (P2P) | コンピューター同士が直接つながる構造、 中央サーバーなし | [1](#1-ビットコインって何) |
| ウォレット | 鍵を保持し、 ビットコインの送受信を可能にするソフトウェア | [3](#3-コインの正体-utxo-モデル) |
| 秘密鍵 | コインを支配する秘密の数値、 これを持つ者がコインを動かせる | [3](#3-コインの正体-utxo-モデル) |
| 公開鍵 | 秘密鍵から導かれる数値、 他人に公開して安全 | [3](#3-コインの正体-utxo-モデル) |
| アドレス | 公開鍵から導かれる短い文字列、 コイン受け取り用 | [3](#3-コインの正体-utxo-モデル) |
| デジタル署名 | 秘密鍵の保有者が何かを認可したことの暗号学的証明 | [3](#3-コインの正体-utxo-モデル) |
| トランザクション | 1 つ以上の入力から 1 つ以上の出力へコインを動かすメッセージ | [3](#3-コインの正体-utxo-モデル) |
| 入力 | これから使う過去の出力への参照 | [3](#3-コインの正体-utxo-モデル) |
| 出力 | 受取人のアドレスに対して施錠された、 新しいコインの塊 | [3](#3-コインの正体-utxo-モデル) |
| UTXO | まだ使われていない出力、 「コイン」 の実体 | [3](#3-コインの正体-utxo-モデル) |
| BTC / satoshi | 単位 (1 BTC = 100,000,000 satoshi) | [3](#3-コインの正体-utxo-モデル) |
| ブロック | トランザクションをひとまとめにした塊 | [4](#4-ブロックチェーン-取引の永久記録) |
| ハッシュ | 任意のデータから計算される短い指紋となる数値 | [4](#4-ブロックチェーン-取引の永久記録) |
| ハッシュ関数 | ハッシュを生成する関数、 入力を 1 文字でも変えるとまったく違うハッシュになる | [4](#4-ブロックチェーン-取引の永久記録) |
| ブロックチェーン | すべてのブロックを古い順にハッシュで連ねた構造 | [4](#4-ブロックチェーン-取引の永久記録) |
| ジェネシスブロック | 最初のブロック (ブロック 0) | [4](#4-ブロックチェーン-取引の永久記録) |
| ブロック高 | チェーン内でのブロックの位置番号 (ジェネシスは高さ 0) | [4](#4-ブロックチェーン-取引の永久記録) |
| マイナー | 新しいブロックを追加しようと試みるノード | [5](#5-マイニング-新規発行と検証) |
| マイニング | 新しいブロックを追加しようとする作業 | [5](#5-マイニング-新規発行と検証) |
| ナンス | 正しいブロックハッシュを探すためにマイナーが変え続ける数値 | [5](#5-マイニング-新規発行と検証) |
| プルーフ・オブ・ワーク (PoW) | ブロックを追加するためにマイナーが解く必要のあるパズル | [5](#5-マイニング-新規発行と検証) |
| 難易度 | そのときのパズルの難しさ | [5](#5-マイニング-新規発行と検証) |
| コインベーストランザクション | ブロック最初の特殊な取引、 新しいビットコインを生み出す | [5](#5-マイニング-新規発行と検証) |
| ブロック報酬 | コインベーストランザクションで新規発行されるビットコイン | [5](#5-マイニング-新規発行と検証) |
| トランザクション手数料 | トランザクションに添えてマイナーへ支払う少額の上乗せ | [5](#5-マイニング-新規発行と検証) |
| マイナー報酬 | ブロック報酬 + そのブロック内の手数料合計 | [5](#5-マイニング-新規発行と検証) |
| 半減期 | 210,000 ブロックごと (約 4 年) にブロック報酬が半分になるイベント | [5](#5-マイニング-新規発行と検証) |
| メモリープール | まだブロックに入っていないトランザクションの待合室 | [6](#6-メモリープール-待合室) |
| 確認 | 自分のトランザクションの上に積まれたブロック数、 多いほど不変性が高い | [6](#6-メモリープール-待合室) |
| 合意 | 全ノードが同じブロックチェーンに同意している状態 | [7](#7-合意と改竄防止) |
| 検証 | ノードがトランザクションやブロックがルールに従っているかを確認する作業 | [7](#7-合意と改竄防止) |
| 二重支払い | 同じコインを 2 回使おうとする行為、 ビットコインが解いた問題 | [7](#7-合意と改竄防止) |
| 改竄 | あとからブロックを書き換えようとする行為 | [7](#7-合意と改竄防止) |
| 最長チェーン | 累積仕事量がもっとも多いチェーン、 正直なノードはこれを採用する | [7](#7-合意と改竄防止) |
...
```mermaid
flowchart LR
  subgraph Inputs[入力 ― 消費する既存の UTXO]
    I1[UTXO A: 0.7 BTC]
    I2[UTXO B: 0.5 BTC]
  end
  TX(("トランザクション<br/>(秘密鍵で署名)"))
  subgraph Outputs[出力 ― 新しく作られる UTXO]
    O1[友人へ: 1.0 BTC]
    O2[自分へお釣り: 0.199 BTC]
    F[手数料: 0.001 BTC<br/>マイナーへ]
  end
  I1 --> TX
  I2 --> TX
  TX --> O1
...
flowchart LR
  ...
  ADDR --> RECV[誰でもこのアドレスに送れる]
  PK --> SIG[このアドレスに施錠されたコインを<br/>動かすトランザクションに署名するのに使う]
```
...
勝ったマイナーは、 ブロックの先頭に特殊なトランザクションを 1 つ入れる権利を得る。 入力を持たない唯一のトランザクション ― **[コインベーストランザクション](/BitcoinArchive/ja/entries/aftermath/2009-01-03-genesis-block/)** だ。 このトランザクションが生み出す出力は、 新しいビットコインがこの世に生まれる唯一の経路だ。 生み出される額を**ブロック報酬**と呼ぶ。 さらにマイナーは、 取り込んだ通常トランザクションに添えられた**トランザクション手数料**もすべて集める。 この 2 つを合わせたものが**マイナー報酬**だ。

```mermaid
flowchart TD
  CB[コインベーストランザクション] --> BR[ブロック報酬<br/>新規発行 BTC]
  TXS[ブロック内の通常トランザクション] --> FEES[手数料合計]
...
```mermaid
flowchart TD
  REAL[本物のチェーン<br/>... B98 - B99 - B100 - B101 - B102 - B103 ...<br/>正直なマイナーが延ばし続ける]
...
すべてのノードが同一の正典チェーンに同意する状態 ― 中央の調停者なしに、 この機械的なルールから自然と立ち現れる状態 ― を**合意**と呼ぶ。
