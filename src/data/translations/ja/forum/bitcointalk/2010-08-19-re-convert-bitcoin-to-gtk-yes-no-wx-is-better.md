...
> wxWidgets自体は問題ではありません。問題は使用されているバージョン（2.9）で、多くのディストロのパッケージ管理者が不安定と見なしています（wxWidgetsの開発者はそうではないと言っていますが）。一方、私の知る限り、wxWidgetsはLinux上ではgtkを使ってすべてを描画しており、Bitcoin開発者がクロスプラットフォーム対応を容易にしています。
