...
sourceNote: "サボの投稿「Bit gold markets」への、Byrne という読者への返信コメント。コメント自体のタイムスタンプ（ページの showComment リンク、コメント ID c3741843833998921269 で確認）は 2008 年 4 月 10 日 03:53 UTC。URL のパス（2008/04）もこれと整合する。投稿本体の表示日付は現在のページ上で異常な値（2008 年 12 月 27 日）を示しており、これはこのコメント自体のタイムスタンプとは別の、未解決のタイムスタンプの食い違いである（サボ=サトシ仮説エントリーの当該異常の議論を参照）。2026-08-22 確認時点で、このコメントを含む同スレッドの全 17 件が「Anonymous」表示になっているが、これはスレッド全体に及ぶ挙動であり、このコメントに固有の証拠ではない。コメント本文は返信先の投稿（「the post above」）について、自分が書いたものとして一人称で言及しており、Bitcoin Magazine の Genesis Files 特集もこの一節をサボ本人の発言として独立に引用している。"
author: "Nick Szabo"
participants:
  - name: "Nick Szabo"
    slug: "nick-szabo"
