...
「タスクバーの代わりにトレイに最小化」する機能が、私のシステムで CPU を食いつぶしていた原因だった。これを無効にしたところ、この暴走 CPU の問題は解消した。

これは 64 ビットクライアントにのみ影響するようで、私が持っている 32 ビットクライアントではこの問題は起きていないようだ。

64 ビットクライアントで気づいたのだが、複数の「トレイ」アイコンが生成され続け、最終的に X サーバーが落ちてしまう。これはどこかにバグとして報告すべきなのだろうか？
