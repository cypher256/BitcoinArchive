...
| 用語 | 意味 | 章 |
|---|---|---|
| ノード | ビットコインソフトウェアを動かしているコンピューター | [1. ビットコインって何？](#1-ビットコインって何) |
| フルノード | ブロックチェーン全体を保持し、 すべてのルールを独立検証するノード (本記事の前提とする種類) | [1. ビットコインって何？](#1-ビットコインって何) |
| 軽量ノード (SPV) | ブロックヘッダーのみダウンロードし、 残りはフルノードに依存するノード (スマートフォン用が典型) | [1. ビットコインって何？](#1-ビットコインって何) |
| ピアツーピア (P2P) | コンピューター同士が直接つながる構造、 中央サーバーなし | [1. ビットコインって何？](#1-ビットコインって何) |
| ウォレット | 鍵を保持し、 ビットコインの送受信を可能にするソフトウェア | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| 秘密鍵 | コインを支配する秘密の数値、 これを持つ者がコインを動かせる | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| 公開鍵 | 秘密鍵から導かれる数値、 他人に公開して安全 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| アドレス | 公開鍵から導かれる短い文字列、 コイン受け取り用 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| デジタル署名 | 秘密鍵の保有者が何かを認可したことの暗号学的証明 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| トランザクション | 1 つ以上の入力から 1 つ以上の出力へコインを動かすメッセージ | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| 入力 | これから使う過去の出力への参照 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| 出力 | 受取人のアドレスに対して施錠された、 新しいコインの塊 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| UTXO | まだ使われていない出力、 「コイン」 の実体 | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| BTC / satoshi | 単位 (1 BTC = 100,000,000 satoshi) | [3. コインの正体: UTXO モデル](#3-コインの正体-utxo-モデル) |
| ブロック | トランザクションをひとまとめにした塊 | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| ハッシュ | 任意のデータから計算される短い指紋となる数値 | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| ハッシュ関数 | ハッシュを生成する関数、 入力を 1 文字でも変えるとまったく違うハッシュになる | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| ブロックチェーン | すべてのブロックを古い順にハッシュで連ねた構造 | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| ジェネシスブロック | 最初のブロック (ブロック 0) | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| ブロック高 | チェーン内でのブロックの位置番号 (ジェネシスは高さ 0) | [4. ブロックチェーン: 取引の永久記録](#4-ブロックチェーン-取引の永久記録) |
| マイナー | 新しいブロックを追加しようと試みるノード | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| マイニング | 新しいブロックを追加しようとする作業 | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| ナンス | 正しいブロックハッシュを探すためにマイナーが変え続ける数値 | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| プルーフ・オブ・ワーク (PoW) | ブロックを追加するためにマイナーが解く必要のあるパズル | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| 難易度 | そのときのパズルの難しさ | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| コインベーストランザクション | ブロック最初の特殊な取引、 新しいビットコインを生み出す | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| ブロック報酬 | コインベーストランザクションで新規発行されるビットコイン | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| トランザクション手数料 | トランザクションに添えてマイナーへ支払う少額の上乗せ | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| マイナー報酬 | ブロック報酬 + そのブロック内の手数料合計 | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| 半減期 | 210,000 ブロックごと (約 4 年) にブロック報酬が半分になるイベント | [5. マイニング: 新規発行と検証](#5-マイニング-新規発行と検証) |
| メモリープール | まだブロックに入っていないトランザクションの待合室 | [6. メモリープール: 待合室](#6-メモリープール-待合室) |
| 確認 | 自分のトランザクションの上に積まれたブロック数、 多いほど不変性が高い | [6. メモリープール: 待合室](#6-メモリープール-待合室) |
| 合意 | 全ノードが同じブロックチェーンに同意している状態 | [7. 合意と改竄防止](#7-合意と改竄防止) |
| 検証 | ノードがトランザクションやブロックがルールに従っているかを確認する作業 | [7. 合意と改竄防止](#7-合意と改竄防止) |
| 二重支払い | 同じコインを 2 回使おうとする行為、 ビットコインが解いた問題 | [7. 合意と改竄防止](#7-合意と改竄防止) |
| 改竄 | あとからブロックを書き換えようとする行為 | [7. 合意と改竄防止](#7-合意と改竄防止) |
| 最長チェーン | 累積仕事量がもっとも多いチェーン、 正直なノードはこれを採用する | [7. 合意と改竄防止](#7-合意と改竄防止) |
...
ここまでが「型」 だ。 ここから先は、 出てきた用語のひとつひとつをより深く追える:
