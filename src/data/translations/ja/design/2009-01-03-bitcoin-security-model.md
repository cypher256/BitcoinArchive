...
各層はそれぞれ独立した仕組みで構成される。暗号学的保証(SHA-256d プルーフオブワーク、ECDSA/Schnorr 署名、マークルツリーコミットメント)、コンセンサスルール(決定論的ブロック検証、最多ワークチェーン選択、難易度調整)、ネットワークアーキテクチャー(ゴシップ中継、ピア多様性、BIP 324 暗号化トランスポート)、経済的インセンティブ(ブロック報酬、ハッシュレートコスト、攻撃者自身の BTC が価値を失うこと)である。
...
|---|---|---|
| **ブロック新規発行分** | 1 ブロックあたり 3.125 BTC（2024 年の半減後） | ブロック報酬の主な構成要素であり、マイナー収入、ひいては攻撃者が買い負けなければならないハッシュレートを賄う |
