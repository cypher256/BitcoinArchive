...
サトシが各候補と実際にどの程度通信したかは、能力プロファイルや文体計量とは独立した観察可能な事実として記録できる。アーカイブ収録の通信を候補ごとに集計すると、対比が明確になる：3 名の候補のみが何らかの形で記録された通信を持ち、6 名の候補はサトシとの直接接触の記録が一切ない。
...
gantt
    ...
    section 候補プロファイル群 A
    アダム・バック （私的 メール 5 通） :2008-08-20, 2009-01-11
    ウェイ・ダイ （私的 メール 3 通） :2008-08-22, 2009-01-11
...
| 種別 | 候補 | 性質 |
|---|---|---|
| **私的メール往復** | アダム・バック、ウェイ・ダイ | サトシがホワイトペーパー公開直前に「先行研究を引用したい」 旨で第三者として接触 |
| **メール＋公開言論** | ハル・フィニー | RPOW 経験者として技術応答を継続。私的メール＋暗号学メーリングリスト返信＋BitcoinTalk 投稿にわたる |
