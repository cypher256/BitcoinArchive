...
姉妹エントリー[サトシ・ナカモトはサイファーパンクではなかった](/BitcoinArchive/ja/entries/analysis/2008-10-31-cypherpunk-independent-arrival/)はサトシの知的立ち位置をサイファーパンク経路に対して扱う。本エントリーは、その経路よりも一段早くに位置する「ハイエクからエクストロピアンへ」の脚を補う。
...
§3.1 の年表が出来事を時間順に並べるのに対し、ここでは同じ連鎖を影響の流れとして示す。単線の時間軸では追えない構造 ― ハイエクから 1990 年代の環境への分岐、そしてその環境のデジタルキャッシュ設計空間からビットコインが立ち上がる流れ ― が見える。

線種は §4 の証拠区分に対応する。実線は実際に作られた・使われたもの ― 開発中に使われた Hashcash と、エクストロピアンが生んだ Hayeks 思考実験。破線は、関連はあるが開発時の直接の入力ではないつながり ― 上流の思想的影響、共同体の重なり、後付けの引用だ。b-money はその後付けの引用にあたる。サトシはアダム・バックに教えられて初めて知り、開発では使っていないが、ホワイトペーパーには引用した。なお Hashcash は通貨ではなく、迷惑メール対策の計算コスト（プルーフ・オブ・ワーク）であって、ビットコインはその仕組みだけを受け継いだ（[Hashcash の発表](/BitcoinArchive/ja/entries/aftermath/1997-03-28-adam-back-hashcash-announcement/)）。だから図でも「デジタルキャッシュの提案」（b-money・Bit Gold・RPOW）とは別に置いてある。Bit Gold と RPOW はホワイトペーパーに引用されていない ― 概念上は近いが（とりわけ Bit Gold）、開発時の入力でも引用でもない。だから提案の箱に残したまま、ビットコインへは線を引いていない。実装レベルの系譜は姉妹エントリー[ビットコインの設計系譜](/BitcoinArchive/ja/entries/analysis/2008-10-31-bitcoin-design-lineage/)が扱う。
