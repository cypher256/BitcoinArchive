...
![ビットコインのタイムロック付きトランザクションスクリプトを表した紺色の技術図解。錠前アイコンが施錠状態から解除可能な状態へ移り変わる様子、時計の文字盤、条件を積んだスタックの箱、そしてエスクローと返金の流れでつながる 2 つのシルエット人物が描かれている。](/BitcoinArchive/images/analysis/2014-10-01-peter-todd-bip-65-checklocktimeverify-hero.png)

<!-- speaker: narrator -->
2014年10月1日、ピーター・トッドは BIP 65 を提案し、ビットコインのスクリプトシステムに新しいオペコード `OP_CHECKLOCKTIMEVERIFY` を導入した。この提案は既存の NOP2 オペコードを再定義し、タイムロックされたトランザクション出力を可能にした。つまり、指定されたブロック高またはタイムスタンプに達するまで使用できない出力にあたる。
