---
title: "Re: WikiLeaksへのビットコイン寄付を歓迎しよう"
date: 2010-12-05T09:08:00Z
source: bitcointalk
sourceUrl: "https://bitcointalk.org/index.php?topic=2007.msg25799#msg25799"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "SatoshiはWikiLeaksの採用を積極的に求めることをビットコインコミュニティに対して控えるよう促し、ビットコインはまだ未成熟であり注目が有害になり得ると主張。"
isSatoshi: true
threadId: "bitcointalk-1735-wikileaks-cia"
threadTitle: "WikiLeaks, CIA, and Bitcoin's future"
threadPosition: 2
tags:
  - "wikileaks"
  - "adoption"
  - "caution"
secondarySources:
  - name: "Satoshi Nakamoto Institute"
    url: "https://satoshi.nakamotoinstitute.org/posts/bitcointalk/521/"
translationStatus: complete
---

いいえ、「歓迎」しないでください。

プロジェクトはソフトウェアを途中で強化できるよう、徐々に成長する必要があります。

WikiLeaksにBitcoinを使おうとしないよう訴えます。Bitcoinは黎明期の小さなベータコミュニティです。小銭程度しか得られないでしょうし、もたらされる注目はこの段階で我々を破壊してしまう可能性が高いです。
