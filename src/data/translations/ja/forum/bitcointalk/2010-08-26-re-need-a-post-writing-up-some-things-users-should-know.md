---
title: "Re: ユーザーが知っておくべきことをまとめた投稿が必要"
date: 2010-08-26T00:44:05.000Z
source: bitcointalk
sourceUrl: "https://bitcointalk.org/index.php?topic=873.msg11227#msg11227"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "スレッド「ユーザーが知っておくべきことをまとめた投稿が必要」におけるサトシ・ナカモトの返信。"
isSatoshi: true
secondarySources:
  - name: "Satoshi Nakamoto Institute"
    url: "https://satoshi.nakamotoinstitute.org/posts/bitcointalk/420/"
threadId: "bt-need-a-post-writing-up-some-things-users-should-kn"
threadPosition: 3
translationStatus: complete
---

どのバックアッププロセス/手順も、ソフトウェアでコーディングソリューションに適切に取り組む時間ができるまでの応急措置にすぎません。コードがそこに到達するまで、言葉で状況を改善しようとできます。

主なバックアップの改善は、事前作成された鍵のプールと、ロード時にブロック履歴から見逃したトランザクションをスクレイピングする再スキャンです。そうすればバックアップは長期間有効に持続します。
