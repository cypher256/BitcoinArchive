---
title: "ネームコインが最初のアルトコインとしてローンチ — ヴィンセント・ダラムが BitDNS 提案を実装 (2011 年 4 月)"
...
participants:
  - name: "Vincent Durham"
    slug: "vincent-durham"
description: "ヴィンセント・ダラムが 2011 年 4 月 18 日にネームコインをローンチ。最初のアルトコインで、BitDNS 提案を実装した分散型の名前登録システム。"
isSatoshi: false
tags:
  - "altcoin"
  - "namecoin"
...
2011 年 4 月 18 日、BitcoinTalk のハンドル名 `vinced` を使う開発者ヴィンセント・ダラムがネームコインのジェネシスブロックを公開した。最初のアルトコインで、独立に動作するネットワークを生んだビットコインコードベース最初のフォークである。
...
- **供給上限**: 2,100 万 NMC、ビットコインと同じ発行曲線
- **ブロック時間**: 10 分 (ビットコインと同じ)
- **PoW**: SHA-256 (ビットコインと同じ)
- **マージマイニング**: ブロック 19,200 (2011 年 10 月) で有効化。これにより、ビットコインのマイナーは追加ハッシュレートなしでネームコインを並行採掘できる

マージマイニングの能力は、ネームコインの設計上もっとも影響を残した選択だった。ビットコインのマイナーに限界費用を発生させないため、ネームコインはゼロから自前のハッシュレートを構築するのではなく、ビットコインのハッシュレートをセキュリティとして借りられた。後続のアルトコインで、ハッシュレート争いを避けつつビットコイン水準のセキュリティを欲するものは、同じ方式を採用した。
