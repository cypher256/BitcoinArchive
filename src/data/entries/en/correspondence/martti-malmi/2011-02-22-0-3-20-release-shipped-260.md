---
title: "Re: 0.3.20 release : shipped"
date: 2011-02-22T19:49:19Z
type: "correspondence"
source: "malmi-email-archive"
sourceUrl: "https://mmalmi.github.io/satoshi/"
sourceNote: "Published on GitHub in February 2024 as part of Martti Malmi's testimony in the COPA v. Wright trial"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
  - name: "Gavin Andresen"
    slug: "gavin-andresen"
  - name: "Martti Malmi"
    slug: "martti-malmi"
description: "Satoshi sends the bitcoin-list mailman admin password to Gavin and Martti as PGP-encrypted blocks. Last email in the Malmi archive (#260)."
isSatoshi: true
tags:
  - "correspondence"
  - "early-contributor"
  - "final-message"
  - "handover"
  - "gavin-andresen"
  - "bitcoin-list"
  - "pgp"
secondarySources:
  - name: "COPA v. Wright Trial Exhibits"
    url: "https://www.opencrypto.org/2024-02-22-witnesses-satoshi-correspondence/"
relatedEntries:
  - aftermath/2011-02-22-satoshi-final-email-to-malmi
quotes:
  - id: "q1"
    person: "Gavin Andresen"
    personSlug: "gavin-andresen"
    date: "2011-02-22T19:00:00Z"
    sourceEntryId: "correspondence/martti-malmi/2011-02-22-0-3-20-release-shipped-gavin-fragment"
---

*[Context: An email from Satoshi addressed to both Gavin Andresen and Martti Malmi (To: both), replying on the "0.3.20 release : shipped" thread. The body labels "Gavin:" and "Martti:" further down are Satoshi's own headings for the two PGP-encrypted blocks — one encrypted to each recipient's public key — not separate addressees.]*

<!-- quote: q1 -->
> I have not sent a message to the sourceforge bitcoin-list mailing list
> because I don't think I have permission; Satoshi, can you give me
> permission, encrypt the mailman password with my public key and send
> it to me, or just post the announcement?

Martti should give you the Drupal admin password.

Any subscriber can post to bitcoin-list.  Here's the admin password in
case you need it later.

Gavin:

```
-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.7 (MingW32) - WinPT 1.2.0


=S7eV
-----END PGP MESSAGE-----
```

Martti:

```
-----BEGIN PGP MESSAGE-----
Version: GnuPG v1.4.7 (MingW32) - WinPT 1.2.0


=0TBL
-----END PGP MESSAGE-----
```
