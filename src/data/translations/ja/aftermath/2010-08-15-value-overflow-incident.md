...
**actor と動機は公開記録からは特定できない。** ブロックチェーンにはトランザクションと 2 つの受取アドレスが記録されているが、作成者の identity は on-chain から復元できない。悪意による攻撃か、概念実証のデモンストレーションか、ストレステストだったかは、コードやトランザクション自体からは判断できない。ブロック 74638 を採掘したマイナーが共謀していたかも不明 — バグのある検証ロジックの下では、正直に動作していたノードもそのトランザクションを「有効」と判定するため、包含は共謀を意味しない。
