---
title: "誰一人として剥がせなかった匿名 — サトシが築いた 6 層の壁"
...
![顔のない人影を中心に、非特定化の 6 つの層を示す同心円状の盾型レイヤーが取り囲んでいる](/BitcoinArchive/images/analysis/satoshi-anonymity-architecture-hero.png)

名前だけなら、サトシが誰だったのかを明らかにする手がかりに見える。だが公開記録は、その謎を 6 つの層に分散させている。日本人風の仮名、匿名リレーを経由したメール、混在する言語と時間の痕跡、Windows ベースの開発環境、伝播の足跡を持たないジェネシスブロック、そして段階的な撤退。どれか 1 つだけを説明しても、記録が突きつける難しさには届かない。

このエントリでは、開発期間（2007 年中頃〜2008 年 8 月）から公の活動期間、撤退までの痕跡を追う。身元候補の比較は[身元仮説総覧](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identity-hypotheses-overview/)で、これだけの材料が残っているのになぜ特定できないのかは[識別の非対称性](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identification-asymmetry/)で扱う。

**6 層の壁 (全体図)**

```mermaid
flowchart TD
  ID["あらゆるサトシ正体仮説<br/>(6 層すべてを整合させる必要)"]
  L1["§1 仮名 (命名層)<br/>日本人風の名<br/>国籍を指さない"]
  L2["§2 通信チャネル (媒体層)<br/>匿名メール 3 種<br/>公開チャネル 5 種"]
  L3["§3 言語的・時間的痕跡<br/>英国式英語<br/>時間帯を跨ぐ投稿"]
  L4["§4 開発・配布の選択 (技術層)<br/>Windows 専用ビルド<br/>.rar / GnuPG MingW32"]
  L5["§5 ジェネシスブロック定数<br/>(起源の脱帰属化)<br/>創造者を証明不能<br/>5 日間の空白"]
  L6["§6 段階的撤退 (退場層)<br/>2010 年 9 月 → 2011 年 4 月<br/>突発でなく漸減"]
  ANON(("非特定化<br/>2008 年 〜 現在"))
  ID -.要排除.-> L1
  ID -.要排除.-> L2
  ID -.要排除.-> L3
  ID -.要排除.-> L4
...
西洋的な人名でも、明らかに技術的なハンドルでも、明示的な匿名エイリアスでもなく、日本人の形をした仮名が選ばれた。この選択は、ある象徴空間の内側に位置する。その空間は、1990 年代後半から 2000 年代初頭にかけての英語圏の SF とハッカー文化のなかで、ネットワーク化された匿名性を東アジア (とりわけ日本) の視覚的・命名的な記号で語るものとして、すでに強く形成されていた。詳細な構造的観察は[テクノオリエンタリズム的読解](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-name-techno-orientalism/)に譲る。

本エントリの目的では、命名層は次の構造的性質を持つ: 暗号学メーリングリストという文脈に置かれた日本人の形をした仮名は、「ネットワークの匿名性 → 日本語的に読める名前 → 国籍不明」という連想空間を生成する。いかなる身元仮説もまずこの象徴空間を分解しなければ、特定の人物にたどり着けない。仮名そのものは特定の国籍・言語的出自を指さず、象徴的な空間を指す。
...
互いに無関係な複数のメールプロバイダー、匿名登録仲介、5 つのチャネルへの投稿の分散 (暗号学メーリングリスト・SourceForge・BitcoinTalk・P2P Foundation・p2p-research) が観察される。このパターンは、いずれか単一のチャネルから一貫した身元プロファイルを組み立てることを許さない構造になっている。

**Tor について。** 2 つの別の問いを区別する必要がある。(a) *サトシは Tor を技術的話題として論じたか?* はい。サトシは BitcoinTalk の「[TOR と I2P](/BitcoinArchive/ja/entries/forum/bitcointalk/topic-22/2010-01-20-re-tor-and-i2p/)」スレッドに参加し (2010 年 1 月 20 日と [2 月 4 日](/BitcoinArchive/ja/entries/forum/bitcointalk/topic-22/2010-02-04-re-tor-and-i2p/)の返信)、`.onion` のバックエンドサポート、Tor SOCKS ポート 9050、Tor プロキシ経由でのビットコインの挙動を技術的に詳しく論じている。Tor を話題として扱ったことは公開記録上確認できる。

(b) *サトシ自身の通信を Tor 経由で送出していたか?* これはサトシの運用プロファイルに関する二次的言説で広く主張されているが、サトシ自身の経路選択についてはサトシ本人による一次資料での明言は確認されていない。この推測は匿名メール + 匿名登録 + 複数チャネル投稿の運用要件に基づくものであって、自己言及によるものではない。本エントリでは個人通信の Tor 経由利用を「妥当に推測されるが直接の確認は取れていない」として扱いつつ、サトシの Tor に対する技術的習熟は検証可能であることを併記する。
...
BitcoinTalk フォーラムの投稿時間は 24 時間に分散しており、UTC 6:00〜12:00 の時間帯にはサトシの投稿活動がほぼ完全に途絶えるという 1 つの一貫した空白を示す。このパターンは BitcoinTalk 公開アーカイブの 575 投稿から導出できる。サトシのソースコードコミット (v0.1.0〜v0.3.19) のタイムスタンプ分析でも同じ時間帯の空白が観察されており、EST (UTC-5) または CST (UTC-6) 在住を示唆する読みが提示されている。その仮定では、空白は現地時間でほぼ 01:00〜07:00 (通常の睡眠時間帯) にあたる。GMT (英国) や JST (日本) の仮定では空白の時間帯が不自然に反転する。コミットパターンの導出については[コード分析エントリ](/BitcoinArchive/ja/entries/analysis/2009-01-09-satoshi-code-analysis/)を参照。
...
| 日付 (UTC) | 出来事 | 出典 |
|---|---|---|
| 2010 年 9 月 | サトシ → ギャビン・アンドレセンへ私的に「他のプロジェクトに取り組んでいる」と伝える。具体的な日付・媒体は公開記録になく、アンドレセンの後年の回想からの再構成。 | ギャビン・アンドレセンのインタビュー (CoinMarketCap「Satoshi Files: ギャビン・アンドレセン」) |
| 2010 年 12 月 3 日 | サトシ → マルッティ・マルミ「ギャビンが適任だ。彼は信頼できる。責任感があり、プロフェッショナルだ。Linux に関しては、技術的に私よりずっと上だ。」 | [`mmalmi.github.io/satoshi` 公開メール](/BitcoinArchive/ja/entries/aftermath/2009-05-02-first-contact-satoshi-reply/) |
...
- **Patoshi マイニングパターンに紐付けられている約 110 万 BTC の処分**（ラーナー 2013、22,503 ブロックにわたる詳細は[識別の非対称性分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identification-asymmetry/)に記載）。観察可能なのは「これらのコインが 2010 年初頭の採掘期間以降オンチェーンで動いていない」ことまで。**観察できないのは**、対応する秘密鍵が保持されているか、失われたか、意図的に破棄されたか、後継者が保持しているか、のいずれであるか。
...
- 2007〜2008 年に公開活動が記録されている能力適合候補 (ハル・フィニー、ニック・サボ) は段階的撤退層 (時間整合) と、ウェイ・ダイの 2014 年の指摘が示す開発期間不可視性の期待で整合しない。
...
- 6 層という整理は一つの読み方である。同じ痕跡を別の軸 (たとえば参加段階順ではなく攻撃面/フォレンジック類で) によって整理することも可能。
- §2 の Tor 推測と §7 の作業地制約の推測は二次文献に依存する。一次記録は沈黙している。本エントリではこれらを推測として扱い、確立した事実としては扱わない。

本エントリで展開した構造的匿名性の読みは、 [サトシの非技術的スルー](/BitcoinArchive/ja/entries/analysis/2010-12-27-satoshi-non-technical-silence/)における行動的匿名性の読みと補完関係にある。同エントリは、クリスマス挨拶や同情の表明、個人的な質問といった非技術的な会話の開きにサトシがどう応答したか、また一方で技術的なやりとりには開いたままであったことを扱う。同一の記録済みパターンを別軸から見た独立分析である。

[サイファーパンク独立到達の分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-cypherpunk-independent-arrival/)は、問いのもう半分を扱う。サトシが誰だったかではなく、サトシがサイファーパンクの経路にどう関わったか、である。

*[補足：本分析が扱う六層の匿名構造は、小説『[ジェネシス ― 創設者の消失と約束](/BitcoinArchive/ja/novel/)』における主人公の沈黙の読解の構造的根拠となる。国籍、年齢、声、顔の意図的な非開示は、小説が中心的な解釈枠組みとして扱う対象である。]*

<!-- entry-closing -->

公開記録が残したのは、失われた一つの手がかりではない。名前、経路、言語と時間、機械、ジェネシスブロック、段階的な撤退という、同じ不在を別々の角度から示す六つの痕跡だ。六つが最初から一つの計画だったとは証明できず、重ねても一つの顔にはならない。それでも、身元を特定したとする主張には、最後まで六つの層すべてを同時に越えることが求められる。
