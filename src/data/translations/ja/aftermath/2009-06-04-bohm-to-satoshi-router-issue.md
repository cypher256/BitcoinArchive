...
このメールで、ボームは新しいルーターを設置した後、ビットコインクライアントがネットワークに接続できなくなったと報告した。本主題 (ルーター交換に伴う接続喪失) は、裁判開始直前に死去したボームが提出した COPA 証人陳述書 (証拠 C/10/1) に記録されており、サトシ・ボーム間のやり取りに言及するメラー判決のパラグラフでも参照されている。

*[編者注：記録するのは、メールの具体的な文面ではなく、文書化された主題と意義である。完全なメール本文は公開されていない — ボームの証人陳述書は裁判証拠 C/10/1 として提出されたが、サトシとの実際のメール交換は公開されているメラー判決 PDF (gwern.net/doc/bitcoin/2024-mellor.pdf) にも、Decashed のノード IP 分析等の COPA 証拠の第三者分析にも、本文として再録されていない。ボームの遺族または COPA が後に逐語的なメール本文を公開した場合は、一次資料方針に従って本文を逐語的に更新する。]*
