...
**初期のマイニングとビットコイン送金：**
Trammellは最初期からビットコインのマイニングを開始し、サトシやハル・フィニーとともにネットワーク上の最初のノードの一つを運用していた可能性がある。2009年1月13日、サトシはテスト取引としてTrammellに25 BTCを送信し、これは最初期の既知の個人間ビットコイン送金の一つとなった（1月12日にサトシがハル・フィニーに送った10 BTCに続く）。やり取りの中で、サトシはコインの成熟ルールやシステムが新しいブロックをどのように処理するかなどの技術的詳細について議論した。

**意義：**
Trammellの早期の採用とサトシとの直接的なやり取りは、彼を最初期のビットコインユーザーの一人として位置付けている。サトシ・ナカモト Instituteに保存されたサトシとのメールは、ネットワークがわずか数台のノードで構成されていたビットコインの最初期の日々を垣間見る窓を提供している。
