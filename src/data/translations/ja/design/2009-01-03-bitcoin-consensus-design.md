...
コードの手順で言えば、このループはブロックヘッダー（前ハッシュ、マークルルート、タイムスタンプ、難易度ビット）を組み立て、その SHA-256d を計算し、結果がターゲット以下かどうかを確かめる。合格すればブロックを配信し、不合格ならナンスをインクリメント（または extra-ナンス用にコインベースを変更）して再度ハッシュを計算する。
...
計算式で言えば、2,016 ブロック窓の最初と最後のブロックの経過時間を、想定時間（2,016 × 600 秒）で割って比率を出し、その比率を [1/4, 4] にクランプし、旧ターゲットにクランプ後の比率を掛けて、次の窓に適用する新ターゲットを得る。
...
上の各関所は個別の検査に対応する: プルーフ・オブ・ワーク（ヘッダーハッシュ ≤ ターゲット）、タイムスタンプ（許容範囲内）、サイズ（ブロックウェイト ≤ 4 MWU）、マークルルート（再計算値に一致）、コインベース（1 件のみ、報酬 ≤ 新規発行分 + 手数料）、トランザクション（署名が有効、参照する UTXO が存在、ブロック内に二重支払いなし）。すべての関所を通過して初めて、チェーン状態が更新される。
...
最も多いフォークは意図された設計ではなく偶然の産物で、2 人のマイナーがほぼ同時に有効なブロックを見つけることで起きる。

<!-- visual: chain-race -->

この自然フォークが解決した後も、生き残った規則自体は次の 2 方向のどちらかへ変わりうる。

| | ソフトフォーク(規則の厳格化) | ハードフォーク(規則の緩和または変更) |
|---|---|---|
| **旧規則への影響** | 新規則により従来*有効*だったブロックの一部が無効化 | 新規則により従来*無効*だったブロックの一部が有効化 |
| **未更新ノード** | 引き続き全新ブロックを受理 — 後方互換 | 更新済みノードが受理するブロックを拒否 |
| **分裂リスク** | ハッシュレート多数派が更新すればなし | 全ノードが更新しない限り恒久的 |
