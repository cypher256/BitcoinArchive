---
title: "ウェイ・ダイの b-money とビットコイン「失敗」判定 — サトシは先に答えていた"
...
2013 年 4 月、[ウェイ・ダイ](/BitcoinArchive/ja/participants/wei-dai/) — b-money 提案がビットコインホワイトペーパーの参考文献 [1] に引用されているサイファーパンク — は、自らが着想を与えた設計に判定を下した。ビットコインの固定供給は金融政策の失敗だ、と。さらに踏み込んで、サトシから 2008 年に届いた白書草稿へのコメント依頼に返信しなかったことを悔やんだ。返信していれば、その考えを思いとどまらせられたかもしれない、と。

この判定は 10 年以上、ビットコイン最大の被引用先行者が固定供給を誤りと考えていた証拠として流通してきた。そこで抜け落ちているのは、その半年前の 2009 年、サトシ自身がなぜ代替案を試みなかったかを自分の言葉で説明していたこと、そしてダイ本人がビットコインより 10 年早くその代替案を提案した際に何が起きたか、である。
...
この返信をそれ自体として読めば、ムーアの法則によるインフレへの反論としては筋が通っている。単位を絶対的な CPU サイクル数ではなく、商品バスケット価格に対して相対的に定義するなら、ハードウェアが安くなるだけでは通貨は毀損しない。だが、この反論は「症状」への反論であって、「仕組み」そのものを改めて主張し直しているにすぎない。依然として「標準商品バスケットに対する CPU 時間の相対コスト」が要る。それは、分散ネットワークがどうやって合意するのか、元の提案が一度も説明していなかったのと同じ、現実世界の価格の数値だ。ダイはバックのインフレへの異議には答えたが、その下にあるもっと難しい問い——「その交換レートが分かっているなら式はインフレを防ぐか」ではなく「そもそもソフトウェアはその交換レートをどうやって知るのか」——には一度も触れていない。
...
物価安定、景気循環、最適インフレ率を、より広い採用を目指す通貨システムなら誰もが直面する未解決の問いとして挙げている——すでに自分が解いた問題としてではなく。15 年後、b-money をなぜ実装しなかったかを振り返った 2014 年の LessWrong スレッドで、ダイはこの設計がまさにその未完成のままだったことを認めている。
...
この設計とその作者について、1998 年 12 月の提案から 2013 年 4 月のダイの判定に至るまでの公開記録のどこにも、ダイ自身にもアダム・バックにも、分散ネットワークが誰かに報告させることなく現実世界の価格をどう決定するかを明示した痕跡はない。b-money は一度も実装されなかった。その間、実働する仕組みは一つも世に出なかった。ダイが 2013 年の判定を書いたとき、彼はサトシが見送った実証済みの代替案を指していたのではない。1998 年以来、自分の設計が必要とし、擁護はしたが閉じることのなかった、その同じ未解決の要件を指していたのだ。

これはダイのビットコインの乱高下への診断が誤りだということではない。彼が名指しした価格変動は現実であり、その先は[固定供給 vs 自動調整通貨分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-fixed-supply-vs-adjustable-money/)がビットコインの上限に対して 15 年分の暗号資産の供給設計を横断し、どちらが正しいかには立場を取らずに整理している。ここで言えるのはもっと狭く、もっと具体的なことだ。2013 年の判定は、代わりに選べたはずの、実装済みで動く代替案という重みを帯びて流通してきた。記録はその重みを支えない。b-money 自身の仕組みは、サトシが「作る方法を知らない」と言ったのと同じものを必要としており、それとは違うことを示せる立場にいたはずの唯一の人物は、ついにそれを示さなかった。

## これが示さないもの

本稿は、分散型の現実世界価値オラクルが不可能であることを立証するものではない。示すのは、ダイがどちらの発言をした時点——1998 年に b-money を設計した時点でも、2013 年にビットコインの設計判断をそれと比べて裁いた時点でも——にも、それが存在しなかったということだけだ。その後の暗号学やオラクルネットワークの研究はここでは扱わない。固定供給が正しい選択だったかどうかも、本稿は決着させない。サトシが弾力供給問題への答えを持たなかったことは、なぜ固定スケジュールを選んだかを説明するが、それだけで固定スケジュールが他の根拠において正しかったことを証明するわけではない。そしてダイ自身の説明は、b-money の開発を続けなかった技術とは別の理由も挙げている。書き上げる頃には「クリプトアナーキーに、ある程度幻滅していた」というものだ。つまり、未解決の仕組みが、この設計がそれ以上進まなかった唯一の理由だったとは限らない。

<!-- entry-closing -->

ウェイ・ダイの名は、ビットコインホワイトペーパーに参考文献 [1] として刻まれている。その金融政策への判定は 10 年以上、資格を備えた異論として——先行者本人が、後継者は間違えたと言っている——流通してきた。その資格は本物だ。だがその異論が拠って立つ仕組みは、その設計者自身が最後まで作り上げなかったものであり、しかもその理由は、提案から 10 日以内に、設計者自身の手で記録に残されている。「もっと賢い方法はなかったのか」というサトシへの問いに、サトシ自身の答えは「作り方を知らないからだ」だった。15年後、その問いを発した本人も、それを知らなかった。
