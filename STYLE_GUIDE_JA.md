...
**改行点には `<br/>` を使う**。 Mermaid は ASCII スペース `U+0020` も暗黙の改行点として扱うが、 日本語の途中に ASCII スペースが入ると**レンダリング時にも単語中の半角空白として表示される** (例: `待機中の トランザクション` → 「待機中の トランザクション」 と空白が残る)。 これは日本語読者にとって不自然なため、 日本語ラベルでは ASCII スペースではなく `<br/>` を使う:
...
✅  ヴラディーミル・<br/>ヴァン・デア・ラーンへ<br/>引き継ぎ
❌  ヴラディーミル・ヴァン・デア・ラーンへ引き継ぎ
```

`・` (中黒) や `、` は改行点に**ならない**。 ASCII スペースは Mermaid 上は改行点として機能するが日本語表示が不自然になるため**非推奨**。 `<br/>` を一律で使う。

しきい値: CJK を含むトークンが**全角 12 文字**を超えると検出対象になる。`<br/>` を挿入するとそこでトークンが分割される。 規則は flowchart / graph / classDiagram / stateDiagram / timeline / gantt 等のラベル系図に適用される。 `title` キーワード行は対象外 (タイトルは別の、より幅の広いレイアウトを用いる)。**sequenceDiagram のメッセージ行は対象外** — メッセージは矢印に沿って水平方向に自動拡張されるため、 列幅オーバーフローの前提が成立しない (ただし表示上の自然さの観点から、 メッセージ内の ASCII スペース挿入はやはり避け、 必要なら `<br/>` を使うこと)。

**固有名の内部では分割しない**。改行挿入位置は固有名 (人名・チェーン名・引用タイトル等) の**外側**を選ぶ。固有名内に `<br/>` を入れると視覚的な断絶 (例: タイトル「ビットコイン実験の終結」 → `ビットコイン<br/>実験の終結` で固有名が割れる) を生む。固有名そのものが span を 12 全角超に押し上げている場合は、ASCII 略号 (`BCH`、`BSV` 等) で span を断ち切るか、言い換えで長い span 自体を回避する:

```text
✗  ビットコイン<br/>実験の終結       (タイトル内分割)
✗  オリジナル<br/>プロトコル          (引用句内分割)
✗  ビットコインキャッシュ<br/>ハードフォーク (固有名と一般語の境界、固有名直後で切れて視覚的に不自然)

✓  ハーンが<br/>ビットコイン離脱を表明      (タイトルを平叙に言い換え)
✓  BCH 分岐                                  (略号 + 一般語、自然な境界)
✓  BSV が BCH から分岐                       (略号で複数 span を断つ)
```

`npm run check:mermaid-ja-wrap` により強制される (`npm run check` の一部として動作する)。スクリプトは `<br/>` (`<br>` / `<br />` 含む) を改行点として認識する。 `npm run check:bios-rendering` (Playwright による視覚確認 — `npm run dev` の実行と `playwright` のインストールが必要) と組み合わせる。
