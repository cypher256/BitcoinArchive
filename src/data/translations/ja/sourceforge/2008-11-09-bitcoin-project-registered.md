---
title: "ビットコインプロジェクトが SourceForge に登録"
date: 2008-11-09T00:00:00Z
type: "forum-post"
source: "sourceforge"
sourceUrl: "https://sourceforge.net/projects/bitcoin/"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "サトシ・ナカモトが SourceForge.net に「ビットコイン」プロジェクトを登録し、ビットコインのソースコードとリリースのための最初の公開リポジトリとプロジェクトページを確立した。"
isSatoshi: true
tags:
  - "sourceforge"
  - "project-registration"
  - "historic"
translationStatus: complete
relatedEntries:
  - aftermath/2009-08-30-bitcoin-svn-repository-committers
---

サトシ・ナカモトは 2008年11月9日に SourceForge.net に「bitcoin」プロジェクトを登録した。これは、2008年10月31日に暗号学メーリングリストにビットコインのホワイトペーパーを公開してからわずか 9日後のことであった。

SourceForge は、ビットコインプロジェクトの主要なホスティングプラットフォームとして機能し、ソースコードのバージョン管理（SVN）、ファイルリリースホスティング、そして bitcoin-list メーリングリストを提供した。sourceforge.net/projects/bitcoin/ にあるプロジェクトページは、ビットコインの初期の開発と配布の中心的なハブとなった。

SourceForge へのプロジェクト登録は、ビットコインをホワイトペーパーに記述された理論的な提案から、共同作業と配布のための公開インフラを備えた実際のソフトウェアプロジェクトに移行させる上での重要なステップであった。v0.1 から v0.3.19 までのすべての初期ビットコインリリースは、SourceForge のファイルホスティングシステムを通じて配布された。
