...
participants:
  ...
  - name: "Wei Dai"
    slug: "wei-dai"
description: "ビットコイン v0.1 は、引用されたサイファーパンクシステム（Hashcash）から 1 つの暗号学的基本要素（プルーフ・オブ・ワーク）を再利用し、複数の汎用コンピューターサイエンス構成要素（マークルツリー、連鎖タイムスタンプ、確率論）を引用文献を経由するか否かを問わず借用し、残りの大半（分散型合意形成、UTXO モデル、マイニング報酬による発行、2,100 万通貨上限、P2P 伝播、ECDSA トランザクション、難易度調整）を独自設計として合成している。本エントリーは、ビットコインの実際の中身と各構成要素の由来を構成要素別に整理し、設計中の使用が確定しているもの（Hashcash、一次資料で確認）、設計後の引用追加が確定しているもの（b-money、一次資料で確認）、汎用 CS 知識の再利用（Merkle・Haber-Stornetta・Feller — 確定不能）、独自合成（引用不能）の 4 区分に分ける。"
isSatoshi: false
tags:
  - "bitcoin"
  - "whitepaper"
...
ビットコイン v0.1 は、引用されたサイファーパンクシステム（Hashcash）から 1 つの暗号学的基本要素（プルーフ・オブ・ワーク）を再利用し、複数の汎用コンピューターサイエンス構成要素（マークルツリー、連鎖タイムスタンプ、確率論）を引用文献を経由するか否かを問わず借用し、残りの大半（分散型合意形成、UTXO モデル、マイニング報酬による発行、2,100 万通貨上限、P2P 伝播、ECDSA トランザクション、難易度調整）を独自設計として合成している。[ビットコインのホワイトペーパー](/BitcoinArchive/ja/entries/emails/cryptography/bitcoin-p2p-e-cash-paper/2008-10-31-bitcoin-p2p-e-cash-paper/) には番号付きの引用文献が 8 件あるが、それぞれの役割は同じではない。本エントリーは、ビットコインの実際の中身と各構成要素の由来を、構成要素別に整理し、設計中の使用が一次資料で確定しているものと、設計後の引用追加が確定しているものと、汎用知識の再利用と、独自合成を区別する。

## 1. 引用の区分

ホワイトペーパーの 8 引用は、サトシの記録された設計過程との関係で 3 つの区分に分かれる。第 4 の区分 — 独自構成要素 — は引用するべき先行が存在しないため、引用文献リストには現れない。

1. **設計中に使用していた（一次資料で確定）**。サトシが当該文献を意識し、その概念を設計期間中に使用していたことが文書化されている。サトシと[アダム・バック](/BitcoinArchive/ja/participants/adam-back/) の 2008 年 8 月 20〜22 日のメール通信が [Hashcash](http://www.hashcash.org/papers/hashcash.pdf) をこの区分に位置づける。
2. **設計後に引用追加（一次資料で確定）**。サトシは設計期間中、当該文献を**知らなかった**。後に知り、ホワイトペーパーの引用文献リストに追加した。2008 年 8 月 22 日のメール通信が [ウェイ・ダイの b-money](http://www.weidai.com/bmoney.txt) をこの区分に位置づける。
...
|---|---|---|---|
| プルーフ・オブ・ワーク | [6] Hashcash（バック 1997 年、2002 年改訂） | 設計中に使用 | 2008 年 8 月 20〜22 日のサトシ↔バックメールで確認 |
...
サトシの [2008 年 8 月 20 日のアダム・バック宛メール](/BitcoinArchive/ja/entries/correspondence/adam-back/2008-08-20-satoshi-to-adam-back/) は Hashcash の正しい引用形式を質問している。本文では初期のビットコイン設計を詳しく述べ、Hashcash を「再利用する既存の基本要素」 として参照している。これは Hashcash の使用が**設計中**であって**設計後**ではないことを位置づける。
...
ホワイトペーパーはブロックヘッダーでマークルツリーを使用し（§7）、タイムスタンプサーバー型のチェーン構築を記述し（§3）、二重支払いの確率分析を行う（§11）。それぞれに引用文献がある。しかし、根底にある概念は当該時期の標準的な 1990〜2000 年代の計算機科学知識であり、相当の大学院レベルの教育を受けた者なら入手可能である。サトシのメール記録には「マークルツリーは知らなかった」 や「連鎖タイムスタンプは知らなかった」 に類する瞬間は含まれない — その不在は、設計中の使用を立証するわけでも、設計後の追加を立証するわけでもない。記録は問いを開いたままにする：サトシは具体的にこれらの論文を参照したかもしれないし、一般的な教養から概念に到達して最も関連する先行として引用を加えたかもしれない。
...
- **b-money の作者であることも、同様にビットコインの設計者であることを意味しない**。b-money の引用はサトシがバックの紹介で知った後に追加されたもので、b-money の概念は設計期間中はビットコインの設計に影響していない。詳細は [正体仮説の概観のウェイ・ダイ・プロファイル](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identity-hypotheses-overview/) を参照。
