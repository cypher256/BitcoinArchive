...
- **マージマイニング**: ブロック 19,200 (2011 年 10 月) で有効化 — ビットコインのマイナーは追加ハッシュレートなしでネームコインを並行採掘できる

マージマイニングの能力は、ネームコインの設計上もっとも影響を残した選択だった。ビットコインのマイナーに限界費用を発生させないため、ネームコインはビットコインのハッシュレートをセキュリティとして借りられた — ゼロから自前のハッシュレートを構築する必要がない。後続のアルトコインで、ハッシュレート争いを避けつつビットコイン水準のセキュリティを欲するものは、同じ方式を採用した。
