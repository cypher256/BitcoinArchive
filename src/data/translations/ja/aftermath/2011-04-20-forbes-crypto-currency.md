---
title: "Forbes「Crypto Currency」 — ビットコインとサトシに関する初の主流メディア特集（2011年）"
date: 2011-04-20T00:00:00Z
type: "article"
source: "forbes"
sourceUrl: "https://www.forbes.com/forbes/2011/0509/technology-psilocybin-bitcoins-gavin-andresen-crypto-currency.html"
author: "Andy Greenberg"
participants:
  - name: "Gavin Andresen"
    slug: "gavin-andresen"
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "ビットコイン関連で初の主要印刷雑誌特集（Forbes）。アンドレセンがビットコインを「金より優れた金」と表現。サトシの撤退に寄与した可能性。"
isSatoshi: false
tags:
  - "forbes"
  - "mainstream-media"
  - "disappearance"
  - "media"
secondarySources: []
relatedEntries:
  - aftermath/2011-10-10-dan-kaminsky-bitcoin-security
  - aftermath/2011-11-23-wired-rise-and-fall-of-bitcoin
  - aftermath/2010-06-11-gavin-andresen-biography
  - aftermath/2011-04-26-satoshi-to-andresen-alert-key
  - aftermath/2011-04-26-andresen-to-satoshi-cia-visit
  - aftermath/2010-07-06-anonymity-and-pseudonymity
inlineLinkKeywords:
  - "海賊通貨"
translationStatus: complete
---

![レトロな雑誌の表紙の隣に、光る硬貨型のメダルや金の延べ棒、カメラのフラッシュのような光に照らされた黒いフード姿の人影、暗い背景に並ぶ 3 つの節目のタイムラインを描いたイラスト。](/BitcoinArchive/images/analysis/2011-04-20-forbes-crypto-currency-hero.png)

*Forbes、2011年4月20日オンライン公開（印刷版 2011年5月9日号）より：*

アンディ・グリーンバーグによる『Crypto Currency』は、ビットコインに関する最初の主要な印刷雑誌記事の一つだった。

[ギャビン・アンドレセン](/BitcoinArchive/ja/participants/gavin-andresen/)は Forbes にこう語った。

<!-- audit:quote-skip -->
> 「ビットコインは、人々の分散型通貨へ我々を戻すために設計されている」
>
> 「これは金よりも優れた金のようなものだ」

記事はビットコイン開発者をギャビンの「サイファーパンクの地下組織」と描写し、匿名のお金に取り組む者たちとして扱った。[サトシ・ナカモト](/BitcoinArchive/ja/participants/satoshi-nakamoto/)はこの描写に異議を唱えたと報じられている。

記事はまた、ビットコインを追跡不可能と描写した。グリーンバーグ自身が後に認めた通り、この評価は誤りであり、暗号通貨はブロックチェーン分析を通じて極めて追跡可能であることが証明された。サトシ自身はその数か月前、ビットコインは真の匿名ではなく仮名にすぎないとマルミに警告しており、その経緯は [2010年7月の匿名性と仮名性に関する警告](/BitcoinArchive/ja/entries/aftermath/2010-07-06-anonymity-and-pseudonymity/)に詳しい。

*[編者注：この記事は 2011年4月下旬のサトシの公のコミュニケーションからの消失に先立つ週に掲載された。記事が生み出した主流メディアの注目がこの時期に寄与したかどうかは未確定である。撤退の兆候はこの記事より 7 か月早く始まっており、この報道への反応というより、既に進行していた撤退の一部と読める。詳しくは[アンドレセンの CIA 訪問開示への返信](/BitcoinArchive/ja/entries/aftermath/2011-04-26-andresen-to-satoshi-cia-visit/)のより詳しい読解を参照。2011年4月26日の[ギャビン・アンドレセンへの最後の既知のメール](/BitcoinArchive/ja/entries/aftermath/2011-04-26-satoshi-to-andresen-alert-key/)で、サトシは書いた：「私のことを謎めいた影の人物として話し続けるのはやめてほしい。メディアはそれを海賊通貨という切り口に変えてしまう」。数日後、CIA 訪問の開示について書いた返信の中で、ギャビン・アンドレセンも同じくこの「海賊通貨」という枠組みを問題視している。詳しくは同エントリを参照。]*
