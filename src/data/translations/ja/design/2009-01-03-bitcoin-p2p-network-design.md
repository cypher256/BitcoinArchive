...
**ゴシップであり構造化ルーティングでない理由。** DHT や階層的トポロジーはメッセージの冗長性を減らすが、単一障害点も生み出し、ネットワークの分断が容易になる。ビットコインのフラットなゴシップモデルは、攻撃者がノードの見る情報を制御するにはそのピアの大部分を侵害しなければならないことを保証する。これが日食攻撃耐性の基盤である。
...
順を追うと、ノードはまず `peers.dat` を試すが、その結果得られる接続数が依然として少なすぎる場合は、集合を補うために DNS シードにも問い合わせる。ハードコードされたシードは、他の 2 つの後にだけ試される段階ではなく、独立した最後の手段として同じ接続ステップに合流する。

**サトシ時代と v27 以降基準の比較。** 元の v0.1 クライアントは主要な発見メカニズムとして IRC に依存していた。これは単純だが、IRC サーバーのダウンタイムやチャネル追放に脆弱な中央集権的アプローチであった。現行の Bitcoin Core は独立したコミュニティメンバーが運営する DNS シードで IRC を置き換えている。DNS シード運営者はネットワークを継続的に探索するクローラーソフトウェアを動作させており、DNS レコードには現在到達可能で互換性のあるプロトコルバージョンを実行しているノードのみが反映される。
...
**出方向と入方向: 設計による非対称性。** ノードは自身の出方向接続を入方向接続よりも信頼する。出方向ピアは自分自身が選んだものだからである。ノードを孤立させたい攻撃者（日食攻撃）が取れる道は二つしかない。すべての出方向ピアを制御するか、攻撃者制御の入方向接続でノードを溢れさせるかである。前者を果たすには、ノードのアドレス選択を予測または操作できなければならない。出方向数の少なさ（合計 10）と独立したアドレス選択アルゴリズムが、これを困難にしている。後者は入方向の退去ロジックによって維持コストがかさむ。
