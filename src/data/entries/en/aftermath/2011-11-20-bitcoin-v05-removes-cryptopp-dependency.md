...
The change came from outside the Satoshi-era contributor pool. **Nils Schneider** (BitcoinTalk handle `tcatm`) — the same contributor Satoshi had credited in the [v0.3.6 release alert](/BitcoinArchive/entries/forum/bitcointalk/topic-626/2010-07-29-alert-upgrade-to-0-3-6/) for a midstate-cache mining optimization in 2010 — opened a branch named `no-cryptopp`. The substantive commit (`6ccff2cb`) on **September 27, 2011** rewrote `SHA256Transform()` in `src/main.cpp` to call OpenSSL's `SHA256_Init` / `SHA256_Update` instead of Crypto++'s `CryptoPP::SHA256::Transform`, removed the `using CryptoPP::ByteReverse` import, and deleted the entire `src/cryptopp/` directory tree (16 files: `sha.cpp`, `sha.h`, `cryptlib.h`, `config.h`, `cpu.cpp`, `cpu.h`, `iterhash.h`, `misc.h`, `obj/.gitignore`, `pch.h`, `secblock.h`, `simple.h`, `smartptr.h`, `stdcpp.h`, `License.txt`, `Readme.txt`). Schneider had received GitHub commit access to `bitcoin/bitcoin` just eight days earlier, on September 19, 2011 — one of five developers granted access that year, as [the site's GitHub-migration committer-access record](/BitcoinArchive/entries/aftermath/2011-09-13-bitcoin-github-migration-committers/) documents in detail.
...
The [Wei Dai = Satoshi hypothesis entry §1.2](/BitcoinArchive/entries/analysis/2008-08-22-wei-dai-satoshi-identity-hypothesis/) frames the codebase dependency as evidence against the null hypothesis that Wei Dai is structurally indistinguishable from other candidates on direct code-contribution grounds. This entry documents the upper bound of that evidence: it ends at v0.5.0.

This Crypto++-removal step is treated as the earlier endpoint of a longer dependency-replacement arc by [the 2016 libsecp256k1 entry](/BitcoinArchive/entries/aftermath/2016-01-15-libsecp256k1-replaces-openssl-bitcoin-core-v012/). That entry reads the two events as a complementary pair — Crypto++ to OpenSSL in 2011, OpenSSL to libsecp256k1 in 2016 — placing this v0.5 step as the first row of that chain, with the v0.12 ship date as its terminal row.
