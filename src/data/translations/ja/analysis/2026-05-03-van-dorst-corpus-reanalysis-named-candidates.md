...
本再分析はさらに三つの角度から、同定の記録全体へと読み戻されている。 [ウェイ・ダイ同定仮説](/BitcoinArchive/ja/entries/analysis/2008-08-22-wei-dai-satoshi-identity-hypothesis/)は本再分析を §3.4 文体距離の定量的な支えとして引用し、 §3 反証要約、 §4、 §5 の限界節で繰り返し参照する。 [ハル・フィニー同定仮説](/BitcoinArchive/ja/entries/analysis/2014-03-25-hal-finney-satoshi-identity-hypothesis/)はフィニー固有の位置 (7 万 5 千著者コーパスにおける上位 6.8 パーセント) を §2.3 の支持的論拠として用い、 §4 でより広い記録の枠組みに置き直し、 §5 では中核的な情報源として参照する。そして[同定非対称性分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identification-asymmetry/)は本再分析を §1.3 文体属性の行の一次資料として扱い、名指し候補に対する同定試行の年表 (マーメイド図) に含め、本再分析の固有の所見 (最近接の名指し候補よりさらに近い無名著者 594 人) を、名指し候補対無名コーパスの非対称性を主張する材料として用いる。
