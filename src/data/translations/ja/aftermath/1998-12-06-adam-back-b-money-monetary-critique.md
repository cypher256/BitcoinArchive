...
本 1998 年 12 月批評は三つの後続分析で精読されている。 [アダム・バック同定仮説](/BitcoinArchive/ja/entries/analysis/2026-04-08-adam-back-satoshi-identity-hypothesis/)はバックが列挙した七つの論点を決定的な証拠として扱っており、 §1.4 では各論点が後にビットコインによってどう解決されるかを対応付け、 §2.1 では同じ列挙を「仮説が説明し切らねばならない第三者的読解の記録」として用いる。 [固定供給対調整可能通貨分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-fixed-supply-vs-adjustable-money/)は同じ批評を別角度から読む。ビットコインの後の固定供給選択が、それに対して下された応答であるところの、サイファーパンクの貨幣論争の記録として扱うのである。 [サイファーパンク独立到達分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-cypherpunk-independent-arrival/)は、バックの中心的提案*「価値を創造するには CPU 時間を燃やす — hashcash と同じだ」*を、サトシがそれをすでに名指ししていた経路を通らずに到達し実装した、PoW を価値に結びつける具体的な配置として読む。
