...
> こんな場面を想像してほしい……
> 
> iPhoneでフルクライアントをバックグラウンドで実行していて、停電が起きる。角の店に向かうと、店主がクーラーの中のものを全部半額セールにしていて、現金かビットコインのみだ。携帯電話のクライアントが店主の携帯電話のクライアントとアドホックBluetoothで接続する。送金通知に署名し（ウォレットに実際の暗号通貨のコインはなく、ブロックチェーンと呼ばれる暗号化された台帳への一連のエントリとしてのみ存在する。実際のコインよりも小切手に近い）、店主のアドレスに送る。店主のクライアントは（しなくてもよいが）ブロックチェーンのコピーを確認して、最後のブロックチェーン更新時にあなたが実際にそのビットコインを所有していたかを確認できる。ローカルで問題なければ、あなたが不正をしていないと想定して取引を受け入れ、半額のミルクを持って帰れる。これは意図的な二重支払いから店主を守るものではないが、これを行うにはかつて正直にコインを所有していた必要がある。停電時にコインを所有していなければ、店主のクライアントは送金を拒否していたはずだ。
