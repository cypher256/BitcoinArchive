...
数人の方にこのスペシャルビルドを実行していただけませんか？ダストスパムトランザクションを免除し、当面の0/未承認問題を解消します。通過させるブロックが1つあれば、以前のトランザクションを解消できます。このビルドでブロックを生成したら投稿してください。

これらはバイナリのみです。Linux版は64ビットのみです。
...
Linux 64ビット版には、64ビットモードのAMD向けcpuid 4-way 128ビットSSE2自動検出の変更が含まれています。テストしてより良いかどうか確認していただけると助かります。
