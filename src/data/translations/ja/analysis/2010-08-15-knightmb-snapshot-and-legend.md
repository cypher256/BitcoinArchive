...
これは、明確な名前を持たない種類のコミュニティ貢献である。彼はコア開発者ではなかった。パッチを書いていたわけでもない。マイニングプールを運営していたわけでもない。本アーカイブの投稿が示す限り、彼はインフラを支え続ける種類の参加者——ハードウェアを横断してテストし、スナップショットをホストし、新規ユーザーの素朴な同期質問に答える——であり、誰の伝記正典にも入らない仕事をしていた人物だった。
...
| 主張の段階 | 種別 | knightmb について何が述べられているか |
|---|---|---|
| 2010-07-18 wobber 投稿（[msg3952](/BitcoinArchive/ja/entries/forum/bitcointalk/topic-431/2010-07-18-knightmb-msg3952/)） | 一次（BitcoinTalk） | ハンドル名で名指さない初期マイナーが ~1,000 コアで稼働し既流通 BTC の ~10% を生成と推定。**wobber が名指したのは William Pitock（ハンドル名「Nenolod」）であり、knightmb ではない** |
| knightmb 同日返信 | 一次（BitcoinTalk） | wobber を引用し、10% の数字を否定：「彼が本当にそんなに生成しているとは 100% 確信できない」 |
| 二次集約サイト・AI 生成伝記 | 二次 | knightmb が個人で約 371,000 BTC（初期供給の約 10%）を保有していたと主張 |
| 比較スケールの確認 | 一次推定 | ラズロ・ハニエツが 2010 年中ごろ 〜90,000 BTC ≈ 流通量の 2-3%、既知最大の早期保有者の一人。10% は 3〜4 倍の規模 |
...
| 証拠の種類 | 公的記録での状態 |
|---|---|
| 連邦の起訴状・判決文に BitcoinTalk のハンドル「knightmb」の記載 | 本アーカイブが参照した二次資料のいずれにも該当なし |
| knightmb 本人による同定を確認する一人称発言 | 公表なし |
| BitcoinTalk 投稿内に Tennessee・本名・米国居住者と対応づけ可能な伝記的詳細 | カタログ化された投稿全体を通じて該当なし（本名・都市・雇用主の記載なし） |
| 二次資料が挙げる状況証拠 | 米国拠点 + 初期ビットコイン活動（両者一致）、2012 年のビットコイン要求（時代的に珍しい設定）、Timekoin との関連（一次資料化されていない） |
