...
名前だけなら、サトシの正体を解く手がかりに見える。だが公開記録が広げているのは、6 つの別々の表面だ。日本人風の仮名、匿名リレーを経由したメール、混在する言語と時間の痕跡、Windows ベースの開発環境、伝播の足跡を持たないジェネシスブロック、そして段階的な撤退。どれか 1 つだけを説明しても、記録が突きつける難しさには届かない。
...
公開記録が支える判断は、「サトシは手がかりを何も残さなかった」というものではない。手がかりは別々に残っているが、ひとつの人物へ収束しない。各層は候補を絞れるが、単独で同定はできない。6 層が最初から一つの計画として設計されたことまでは証明できず、その動機も復元できない。それでも、身元を主張するなら同時に越えなければならない境界は、ここに保存されている。
