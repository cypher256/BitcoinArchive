...
participants:
  ...
  - name: "Matt Corallo"
    slug: "matt-corallo"
description: "標準化された支払いリクエスト用の「bitcoin:」URI スキームを定義。クリック可能アドレスと QR コードに金額・ラベル・メッセージを含められ、使いやすい決済の基盤となった。"
isSatoshi: false
tags:
  - "bip"
  - "uri-scheme"
...
スキーム成分 (「bitcoin:」 ) は大文字小文字を区別せず、実装は大文字小文字の任意の組み合わせを受け入れなければならない。URI のそれ以外の部分は、クエリパラメーターのキーを含め、大文字小文字を区別する。
