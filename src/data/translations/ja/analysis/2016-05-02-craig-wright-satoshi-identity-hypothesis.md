...
反論：それはジェネシスブロックのコインベース鍵には決して及ばなかった。その鍵による署名こそが、決定打となるただ一つの実演である。[ジェネシスブロックハードコード分析](/BitcoinArchive/ja/entries/analysis/2009-01-03-genesis-block-hardcode-analysis/)が指摘するとおり、その署名はライトを含め誰も行っていない。1〜9 番の署名を主張しながら、問いを決する鍵を持つ唯一のブロックの手前で止まる論は、問いを未決のまま残す。
