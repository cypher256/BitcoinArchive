---
title: "Forbes「Crypto Currency」 — ビットコインとサトシに関する初の主流メディア特集（2011年）"
date: 2011-04-20T00:00:00Z
type: "article"
source: "forbes"
sourceUrl: "https://www.forbes.com/forbes/2011/0509/technology-psilocybin-bitcoins-gavin-andresen-crypto-currency.html"
author: "Andy Greenberg"
participants:
  - name: "Gavin Andresen"
    slug: "gavin-andresen"
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "ビットコイン関連で初の主要印刷雑誌特集（Forbes）。アンドレセンがビットコインを「金より優れた金」と表現。サトシの撤退に寄与した可能性。"
isSatoshi: false
tags:
  - "forbes"
  - "gavin-andresen"
  - "mainstream-media"
  - "disappearance"
  - "media"
secondarySources: []
relatedEntries:
  - aftermath/2011-10-10-dan-kaminsky-bitcoin-security
  - aftermath/2011-11-23-wired-rise-and-fall-of-bitcoin
  - aftermath/2010-06-11-gavin-andresen-biography
  - correspondence/gavin-andresen/2011-04-26-satoshi-to-andresen-alert-key
translationStatus: complete
---

*Forbes、2011年4月20日オンライン公開（印刷版2011年5月9日号）より：*

アンディ・グリーンバーグによる「Crypto Currency」は、ビットコインに関する最初の主要な印刷雑誌記事の一つだった。

[ギャビン・アンドレセン](/BitcoinArchive/ja/participants/gavin-andresen/)はForbesにこう語った。

> 「ビットコインは、人々の分散型通貨へ我々を戻すために設計されている」
>
> 「これは金よりも優れた金のようなものだ」

記事はビットコイン開発者をギャビンの「サイファーパンクの地下組織」と描写し、匿名のお金を政府の監視の外で違法薬物の購入に使うものとした。[サトシ・ナカモト](/BitcoinArchive/ja/participants/satoshi-nakamoto/)はこの描写に異議を唱えたと報じられている。

記事はまた、ビットコインを追跡不可能と描写した — グリーンバーグ自身が後に認めた通り、この評価は誤りであり、暗号通貨はブロックチェーン分析を通じて極めて追跡可能であることが証明された。

*[編者注：この記事は2011年4月下旬のサトシの公のコミュニケーションからの消失に先立つ週に掲載された。記事が生み出した主流メディアの注目と違法行為との関連付けが、サトシの撤退の決断に寄与した可能性がある。2011年4月26日の[ギャビン・アンドレセンへの最後の既知のメール](/BitcoinArchive/ja/entries/correspondence/gavin-andresen/2011-04-26-satoshi-to-andresen-alert-key/)で、サトシは書いた：「私のことを謎めいた影の人物として話し続けるのはやめてほしい。メディアはそれを海賊通貨という切り口に変えてしまう」]*
