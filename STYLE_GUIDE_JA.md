...
ソースコメントの言語は、そのページ・ファイルのロケールに合わせる。**既定は英語** — コミットメッセージや正典ソースと同じく、最も広い貢献者が読めるようにするため。EN ロケールのページ (`src/pages/` 直下で `ja/` 以外)・共有コンポーネント・スクリプト・スタイルは英語 (混在する旧来分は、触れた折に英語へ正規化。一括書き換えはしない)。

**例外: 日本語版ロケールのページ (`src/pages/ja/**`) は日本語コメントを用いてよい。** 保守者が日本語で読むうえ、EN/JA のページ対はロジックの鏡であってコメントの鏡ではない。
...
### `translationStatus` の判定基準

frontmatter の `translationStatus` は実態に正直であること。「将来補う」 等の言い訳注記を添えて完了状態を立てる運用は禁止する。意味:

- `complete`: 本文の散文を含めて**すべて翻訳済**である場合に限る。見出しだけ訳して本文の散文が英文のまま残っている状態は `partial` であり、`complete` を立てない
- `partial`: 一部のみ訳された状態 (見出しのみ翻訳・序文だけ翻訳・節単位の部分翻訳・特定箇所の言い換え未着手 等)
- `pending`: 翻訳に未着手
- `machine`: 機械翻訳 (未校正)

`partial` 状態のエントリーは、本文先頭にブロック引用形式の警告を置き、未訳箇所を読者に明示する:

```markdown
> **部分翻訳警告 — 散文は未訳**
>
> 本記事は見出しのみが日本語化されており、本文の散文は英文のまま保持されている。完訳は段階別の翻訳計画で順次補完される予定。
>
> 原文: <URL>
```

警告の文言は記事固有の事情に合わせて調整してよい (例: 「序文と要約のみ翻訳済」「節単位で 4/8 翻訳済」「数式・コード・テストベクトルは技術精度のため翻訳対象外」 等) が、必ず以下を含める:

- 何が訳されていないか (散文 / 特定節 / 特定箇所)
- なぜ未訳のままか (翻訳予定 / 技術的事情で原文保持 / 等)
- 原文へのリンク

過去事例（詳細: `translation-status-false-complete.md`, incidents/）: 9 BIP JA エントリーが虚偽の`complete`表示を起こし、警告形式をブロック引用+太字リードへ格上げした。

### 英語原文と日本語訳の併記は禁止 (例外あり)

日本語ページに英語原文と日本語訳を上下に並べて両方掲載することは禁止する。日本語訳のみを置く。英語原文を読みたい利用者は言語切替で英語版エントリーへ移動できる。両方並べると重複が肥大し、本文の流れも切れる。

例外は、**英語原文そのものが歴史的記録として国際的に固有名化している短い引用** に限る。サトシの最終メールのような、英語の語形が記号として広く参照されているものが該当する。判断の目安:

- その英語の語形が複数の二次資料で繰り返し引用されているか
- 訳のみでは読み手がどの記録を指しているか同定しにくいか
- 短いか (1 ~ 2 文。長文書簡やプレスリリースは対象外)

三つすべてを満たす場合に限り、英語と日本語を併記してよい。一つでも欠ければ日本語訳のみに留める。
...
- メーリングリスト・フォーラム投稿の長文引用
- プレスリリース・公式声明の本文
- ホワイトペーパー・BIP・PR レビューの段落
- 一般的なメール書簡、Q&A、討論ログ

迷ったら日本語訳のみに留める。例外を増やす方向には倒さない。
...
- **原文保持が必要な引用フレーズ** (人物が実際に発した語句、草稿と最終版の語句対比等。書名ではない) は `『…』` へ変換しない。既存の記法 (`「…」` または `"..."`) のまま保持する — 例: サボの言い間違えの書き起こし「I designed bitcoi…gold with two layers because…」。`scripts/audit-en-inline-in-ja.mjs` (`npm run audit:en-inline-in-ja`) が検出する JA 本文インラインの英語断片は、識別子相当・書名相当のどちらにも当てはまらない場合がある。この第三の扱いの確立経緯は `en-inline-third-category-established.md`(incidents/)。
...
### 構造化 chip が付いた引用元は、生の引用先表示行を本文に残さない

`<!-- quote: qN -->` で chip 化された引用の直前に、同じ引用元を示す生の引用先表示行 ( `[Quote from: NAME on DATE]`、`NAME wrote:`、`On DATE, NAME wrote:` 等 ) を残してはならない。chip 自体がアバター・名前・日付・出典リンクを表示するため、直前の生テキストは同じ情報の重複描画になる。

BitcoinTalk 由来の引用は元から一貫してこの扱いだった ( 生の `[Quote from: NAME]` は chip 化と同時に除去、削除済みソースで chip を張れない場合のみ `<!-- audit:quote-skip -->` と共に残す )。書簡・メーリングリストの `NAME wrote:` 系だけがこの扱いから漏れていた事故の詳細は `correspondence-quote-header-not-stripped.md`(incidents/)。

**除去してよいのは、chip がその生テキストの示す引用元と一致する場合のみ**。以下は除去しない:

- aftermath / 伝記散文の「サトシは次のように書いた：」 のような**通常の日本語散文**の導入文 ( 上記「散文中の引用先表示 vs 構造的な引用先表示」 参照。翻訳対象の散文であり構造マーカーではない )
- 削除済みソース・外部引用で chip が張れない場合の生テキスト ( `<!-- audit:quote-skip -->` と共に残す。除去すると帰属が読者に伝わらなくなる )
- 一次資料本文の地の文として意味を持つ英文引用 ( 例: 書籍からの引用「As X and Y wrote: "..."」等、構造的な引用元表示ではなく通常の英語散文の一部 )
...
**削除済みラベル**: 削除済みソースで BitcoinTalk 由来の `Quote from: NAME on DATE` ヘッダー行を保持する場合、 リンク先が消滅しているため平文で残し、 リンクが無い理由を示す接頭辞を付ける。 表記は **EN ページ `[Deleted]`、 JA ページ `[削除済み]`** ( この接頭辞は原文の一部ではなく Archive の編集注記なので JA 側は日本語化する。 `Quote from:` 以降は原文 UI の逐語保存なので両言語とも英語のまま )。確立の経緯: `deleted-source-label-established.md`(incidents/)。
...
2. 既にチップ済みの論理ソースを再度指す `<!-- quote: qN -->` の繰り返し (その人物のソースが 1 件のみの場合) — 繰り返しチップはタグに降格される（詳細: `duplicate-qn-legacy-rendering-fix.md`, incidents/）。
...
典型例 (1 箇所が本文、もう 1 箇所が引用ブロック):
- ある参加者 X が発言 → エントリー A で X 自身の原文として記録される (本文)。
- 別の参加者 Y がその発言を引用 → エントリー B の `>` 引用ブロックに再掲される。
- このとき、エントリー A の日本語訳とエントリー B の `>` ブロック内日本語訳は**同一の文言**にする。

両方とも引用ブロックの場合 (= 複数のエントリーが同じ発言を引用) も同じく統一する。

両方とも本文の場合 (= 同じリリース告知文を複数の場に重複掲載した等) は本ルールの対象外。重複本文の品質チェックは `check-source-duplication` 系が担う。

過去の違反例(本節のルール確立の契機、詳細: `quote-translation-consistency-unified.md`, incidents/): 同一英文の訳語が2エントリー間でずれていた。現在は両ファイルとも統一形。

例外: 文脈の必然で訳し分ける場合 (= 同じ英文が異なる意味文脈で使われる) は許容。判断に迷うときは、より正確で自然な訳に統一する。

統一先の選び方 (上記の例外に当たる場合を除く): ずれている掲載箇所のいずれかが一次資料エントリー (その発言を逐語保存する primary-source 型) にある場合は、一次資料側の訳を正典とし、他方をそちらへ揃える (一次資料側の訳自体に誤り・欠落が確認された場合は、原文と照合して先に一次資料側を正してから揃える)。同一エントリー内で引用ブロックと本文中の再引用 (「…」 による部分引用) がずれている場合も同じ — 再引用は、そのページの引用ブロック、または一次資料の正典訳に一致させる (部分引用は段落単位の機械検出に掛からないことがあるため、執筆時に目視で揃える)。

検証: `npm run check:quote-translation-consistency` (= `scripts/check-quote-translation-consistency.mjs`) が「少なくとも 1 箇所が `>` 引用ブロックで使われている同一英語段落の訳語ずれ」 を段落単位で検出する。表記揺れは本スクリプトの正規化で吸収するので、報告は実質的な訳語の差にほぼ絞られる。`npm run check` と build に組み込まれており、訳語ずれが 1 件でもあれば build が失敗する(昇格の経緯: `quote-translation-consistency-unified.md`, incidents/)。新規翻訳の追加直後や、ずれを修正する作業の前後にも手動で実行して確認できる。
...
**editorial entries ( `article` / `analysis` / `biography` / `design` / `currency` / `guide` ) の場合:**

- 本文相当の事実 ( 関連事実、 後日譚、 経歴、 続報、 現代価値換算、 節導入文 ) は **本文に地の文として書く**。 Role C / D 枠に押し込まない。
- **判定基準は文の位置や反復の有無ではなく、その文の機能。** その一文が、いま書いている節の議論・比較・結論を前進させるなら地の文 — 複数の並行する節それぞれの結びとして同じ形の案内が繰り返される場合も含む ( 例: 比較記事が各節の最後を「詳細は個別の設計文書へ」で締める場合、その案内は本文構造そのものの一部であり Role D の余談ではない )。逆に、その文が本文の議論から読者を引き離し、別の場所へ誘導するだけなら Role D — 本文の書き出しより前に一度だけ置かれる案内、または連続する段落の間に挿入される別主題のポインタがこれに当たる。反復は構造的機能の目安にすぎず、必須条件ではない ( 単発でも地の文になりうるし、反復していても D が要る場合がある )。判定に迷ったら、角括弧やラベルを外してただの一文として読み、周囲の議論を前進させるか、それとも読者を他所へ導くだけかを確認する。
- Role C / D 枠を使うのは:
  - Role C ( `*[編者注：...]*` ): 証拠や主張をどう読むべきかを限定する編者の解釈・留保 ( 未検証情報の明示、 復元の限界、 適用範囲の明示等 )。単に重要な事実であることは C にする理由にならない — 重要な事実ほど地の文に書く。
  - Role D ( `*[補足：...]*` ): 本文の議論そのものには属さない別主題のコンテクスト、 または関連エントリーへの独立したポインタ ( 本文に書くと流れを切る場合 )
- 真に補足的な内容 ( メタ的な編集方針宣言、 出典信頼性メモ、 「今後の調査で更新」 等の TODO ) は Role C / D 枠維持。
- **検出はスクリプト、分類は人手。** `check-editorial-markers.mjs` はラベル付き `*[...]*` の正規形式だけを検査し、ラベルの無い生の斜体 ( 書名や用語の強調等が大半 ) を地の文 / C / D に自動分類する責務は持たない。個別の一文への適用は編集時の人手判断のまま。
...
エントリーの題材が小説『[ジェネシス ― 創設者の消失と約束](/BitcoinArchive/ja/novel/)』
でどう扱われるかを示す短い導線注は、 通常の `*[補足：...]*` (Role D) を本文末尾に置き、
`/BitcoinArchive/ja/novel/` へのリンクを含める。 この枠内では小説由来の意味づけを書いてよいが、
枠内の内容は史実根拠ではなく、 他の Archive 本文へ独立出典なしに再利用してはならない。
枠外では通常どおり、 小説内容禁止ルールと史実出典ルールを適用する。

**過去事例（詳細: `dan-kaminsky-context-to-body.md`, incidents/）:** 補足枠に押し込まれた本文相当の事実を地の文化した典型例。
...
EN ソースから JA に翻訳するとき、 英文の構造 (語順・指示詞・抽象名詞・否定の重ね方) をそのまま日本語に写し取ってはならない。 意味は通っても日本語として不自然な文になる。 一文ごとに「日本語ネイティブが読んで違和感がないか」 を問い、 違和感があれば言い回しを組み直す。

本節は §3 (翻訳の完全性)、 §6 (語調・話者性)、 §8 (技術用語の日本語化) と並ぶ翻訳の品質規則。 §8 が「英語の単語をどう日本語化するか」 を扱うのに対し、 本節は「英文の構造をそのまま日本語に持ち込まない」 を扱う。

### 典型的な直訳調パターン (避ける)

| 英文構造 | 直訳調 ❌ | 自然な日本語 ✅ |
|---|---|---|
| `The chapters above are the model` | `ここまでが「型」 だ` | `ここまでがビットコインの基本的な仕組みだ` / `以上が全体像だ` |
| `assumes you know nothing about X, nothing about Y, nothing about Z` | `X も Y も Z も、 何ひとつ知らない読者を前提にしている` | `X、 Y、 Z のどれも前もって知っている必要はない` |
| `expand X across N areas` | `X を N 領域に展開した` | `X を N 領域に整理した` / `N 領域に分けて記述した` |
| `Start here for...` | `〜したい人はここから` | `〜したいときはここから始めるとよい` / `〜の起点` |
| `none of X / neither of X` | `いずれの X も〜ない` | `X のどれも〜ない` / `どの X も〜ない` |
| `assume no prior knowledge` | `前提知識を求めない` | `予備知識は要らない` / `前もって知っている必要はない` |
| `every term you saw can be followed deeper` | `出てきた用語のひとつひとつをより深く追える` (直訳調) | `出てきた用語のひとつひとつをもっと詳しく辿れる` (文脈次第) |
| `ship the codebase / ship a fix / ship v0.x / an idea gets shipped` (ソフト・コード文脈の `ship`) | `コードベースを出荷した` / `修正を出荷した` / `v0.x を出荷した` / `アイデアが出荷される` | `コードベースを公開した・世に出した` / `修正を公開した` / `v0.x をリリースした・公開した` / `アイデアが形になる・実装される` (詳細: `ship-literal-translation-fixed.md`, incidents/) |

**`ship` の例外 — 温存 (直さない):** 物理ハードウェア・物品に対する「出荷」 (例: ASIC・マイニング機器・コンピューターが出荷される) は直訳ではなく正しい自然な日本語であり、そのまま残す。ソフト/コード文脈か物理文脈かで判定する。該当例: `aftermath/2011-10-13-litecoin-launch.md`、`aftermath/2011-08-01-jihan-wu-biography.md`、`correspondence/mike-hearn/questions/2009-04-12-satoshi-to-hearn-scalability.md`、`forum/bitcointalk/topic-645/2010-08-05-olipro-msg7693.md`。

本表は例示であり網羅ではない。 新パターンには下記「自然化のチェック手順」 を適用する。

### 自然化のチェック手順

1. EN を JA に訳した直後、 一文ごとに頭の中で読み上げる
2. 「日本語ネイティブが読んで自然か」 を問う
3. 「型」「展開」「いずれの〜も」「〜を前提にしている」 等の硬い熟語・直訳熟語があれば言い換える
4. 直訳調の典型パターン (上表) に該当しないか確認する
5. 違和感が残るなら、 英文を忘れて「日本語でこの内容をどう書くか」 をゼロから書き直す

### 手紙・メールの結びの言葉と署名 (構造レベルの直訳)

上記の表は文単位の直訳調を扱うが、同じ「英文の構造をそのまま持ち込む」問題は文単位を超えて、一次資料 (メール・書簡) の結びにも起きる。英語の手紙・メールは「結びの言葉,」の直後の行に差出人本人の署名を置く形式が定着しているが、これをそのまま日本語に移植し、結びの言葉と名前をカンマ+改行だけで繋げると、名前が結びの言葉に対する**呼びかけの相手**に見える (例:「よろしく頼む、」の直後に「Satoshi」があると、誰かがサトシに向かって「よろしく頼む」と言っているように読める — 実際はサトシ自身の署名)。

**対応:** 結びの言葉は句点 (`。`) で言い切り、名前は**空行を挟んで**独立した行に置く。英語の「結びの言葉,\n名前」という一体の構造を、日本語では「結びの文。\n\n名前」という二つの独立した要素に分ける。これは訳語の選び方の問題ではなく、構造の問題 — どんなに自然な結びの言葉を選んでも、名前がカンマで直結していれば同じ誤読リスクが残る。

正しい例 (この corpus 内の既存訳、変更不要):
...
結びの言葉自体の訳語は、まず英語原文が実際に何を言っているか (感謝なのか、一般的な結びの定型句なのか) を見て選び、そのうえで話者の口調規則 (§ I.6・§ I.7) に合わせる。`Thanks,` / `Thank you.` のように原文が感謝を表している場合は「ありがとう。」のように感謝を直接表す訳を使う — だ/である 調だからといって、意味の異なる定型句に置き換えてよいわけではない。「頼む」は特に避ける: 依頼の動詞であり、結びの定型句としては原文にない意味を足す上、直後の名前をより強く「呼びかけ」に見せる。

**過去の違反例・本節確立の契機（詳細: `letter-closing-mechanical-tone-conversion.md`, incidents/）:** 口調統一の一括修正が結びの言葉を機械的に置換し、意味変化と誤読リスクを発生させた。

### 禁止される逃げ

- 「英文の意味は正確に伝わっているから OK」 と判断する (= 自然さを犠牲にする)
- 「英文の構造を保つ方が原典に忠実」 と判断する (= 日本語として不自然なら原典忠実より自然さ優先)
- 「直訳でもそのうち読者が慣れる」 と判断する (= 読者は直訳調を不快に感じて離脱する)

### audit の合格判定前チェック

第三者auditの合格判定が信頼できなかった事故の詳細: `audit-pass-not-quality-guarantee.md`(incidents/)。合格判定を出す前に以下を確認する:

1. カタカナ語の必然性 — 定着カタカナ (§ I.1 参照) か、和語に置き換えられる不要なカタカナ化かを確認する
2. 役職名・概念語の英語残置 — 肩書き・専門用語が翻訳されず英語のまま残っていないか確認する
3. 「〜的」+ 抽象名詞の組み合わせの違和感 — 直訳調の典型 (例:「効果的な」「体系的な」 の乱用) がないか確認する

### 過去の違反例 (履歴として保持、 本表自体は拡張しない)

読者導線ページの直訳調放置事故（本節確立の契機。詳細: `visual-glossary-literal-translation-origin.md`, incidents/）。

## 10. 語感の逸脱 — 文法的に正しくても事実と食い違う語

翻訳として意味が通り、文法的にも正しい語が、日本語での含意のレベルで記録された事実と食い違うことがある。§ 9 が英文の構造の写し取りを扱うのに対し、本節は一語の含意のずれを扱う。

典型例: 英語の `worked with X` を「X と働いた」と訳すと、日本語の「働く」は文脈によって雇用・職業的労働を想起させる。「協力した」「共に開発した」など、記録された関係の性質に合う動詞を選ぶ（詳細: `worked-with-employment-implication.md`, incidents/）。

チェック手順: 人物間の関係・行為を表す動詞を訳すとき、その日本語動詞が含意する関係 (雇用か無償か、対等か上下か、公的か私的か) が一次資料の記録と一致するかを一度確認する。該当語の網羅は不可能なため、語のリストではなくこの確認手順で運用する (§ 8・§ 9 と同じ原則)。

## 11. 同形異義の一語 — 「金」の money / gold 曖昧性

「金」は文脈により「かね (money)」「きん (gold)」のどちらにも読め、複合語の外に裸で置かれると読者が読み違える。本アーカイブは「デジタルゴールド」としてのビットコインと、資金・資本としてのビットコインの両方を頻繁に、しばしば同じ節内で扱うため、この曖昧性がとりわけ起きやすい。

実例（本節の由来。詳細: `kane-money-gold-ambiguity-origin.md`, incidents/）: 分析ページの見出しで「金」が読み違えられた。gold の意味では、金価格・金 ETF・金相場のように後続の名詞が読みを固定する複合語の中で裸の「金」のままにする一方、ビットコインとの直接比較のような単独の名詞として置く場面では「ゴールド」を使う。

運用:
- money の意味では、裸の「金」を書かない。「資金」(だ／である調の論説での既定) または「マネー」(市場報道寄りの語感、見出しにも使える) を使う。「お金」は曖昧性を解消するが話し言葉寄りの語感を持つため、だ／である調の論説では避ける。
- gold の意味の「金」は、金価格・金 ETF・金相場等、後続の名詞が読みを固定する複合語の中ではそのままでよい。単独の名詞として置く場合、特に見出し・初出で、同じ節に money の話題があるときは「金（ゴールド）」と一度注記するか、「ゴールド」を直接使う。
- 該当語の網羅は不可能なため、語のリストではなく上記の判定手順で運用する (§ 8・§ 9・§ 10 と同じ原則)。
...
- ✓ `Bitcoin XT がブロックサイズ戦争を開く — ハーンとアンドレセンが 8 MB ブロックを提案 (2015 年 8 月)`
- 強い主題句のあとに `：` を置き、可算的な列挙 (人数・件数・年代等) と争点を続ける構造が、読み手と AI の双方にとって引用しやすい。
  - `サトシ・ナカモトは誰か：10 人の天才たちと世紀のミステリー`
  - `ビットコインの家系図 — 分岐、アルトコイン、本流として残ったビットコイン`
  - 当てはめ先がない題材に無理に当て込むと、目録ではなく宣伝のように読める。その場合は記述的な平叙のタイトルを選ぶ。
- 公開されるアーカイブのため、感嘆符・煽り表現・宣伝口調は避ける。読み上げて自然に響くタイトルが目標であり、目立たせるためのタイトルではない。
...
| `aftermath/`、`bip`、`analysis`、`design` | 該当なし (スレッド構造がない) | 個別に編集 |
...
```
EN: BitDNS and Generalizing Bitcoin
JA: BitDNSとBitcoinの汎用化

EN: Ethereum: Welcome to the Beginning
JA: イーサリアム: 始まりへようこそ
```

これは § I.3「日本語ページの散文はすべて日本語」 + 起点投稿タイトル方針 + 既存 `forum/*` JA starter (BitDNS、`Major Meltdown` 系のトピック等) の慣例を統合した規則。`Re:` 返信のタイトルは起点の翻訳タイトルにカスケードされる (上記カスケード表参照)。

例外 (a) 文脈投稿への返信、(b) 件名逸脱の返信は § カスケード規則の例外 (フォーラムスレッド) を参照。
...
**補足（語中の複合語は対象外）:** 末尾長音符の規則は**語末**の省略を対象とする。短い形が語中に現れて正規の複合語をなす場合は対象外 — 例:「フィルタリング」(filtering) は正しい形で、「フィルターリング」 に直さない。`check-ja-glossary.mjs` の `フィルタ` ルールは `except: ['リング']` でこの語を除外する（同種の語が増えたら `except` に追記する）。
...
| block subsidy / subsidy (新規発行コイン部分) | 新規発行分 | 補助金 / ブロック補助金 | 日本主流のビットコイン解説サイト 7 サイト (Wikipedia ja / Coincheck / GMO コイン / SBI VC / bitFlyer / CoinDesk Japan / jpbitcoin) の調査で `補助金` は実用例ゼロ。政府助成金の語感が強く誤解を招くため非推奨。idiom: `subsidy era` → `新規発行時代` / `新規発行分のあった時代`、`post-subsidy era` → `新規発行終了後の時代`、`subsidy contracts/declines` → `新規発行分が縮小`。`mining-reward block-subsidy issuance` 複合表現は `マイニング報酬による新規発行` で統一 (`bitcoin-design-lineage` / `szabo-satoshi-identity-hypothesis` / `adam-back-satoshi-identity-hypothesis` / `b-money-monetary-critique` で同形)。コード識別子 `nSubsidy` / `GetBlockSubsidy` 等は § I.2 区分 3 で英語形維持。`subsidize/subsidized` の本来語義 (政府助成金等) は `補助 / 補助金付き` を維持 (`aftermath/2018-10-01-ray-dillinger-interview.md` の `subsidized rates for electricity` → `補助金付き電力料金` 等) |
...
| (transaction) malleability | (トランザクション)展性 | 順応性 / malleability (本文中) | 2026-06-30 自然化監査で確定 (コミット `376d4479f`) |
| value overflow (incident) | バリュー・オーバーフロー(事件) | 値オーバーフロー | 事件名タイトル `aftermath/2010-08-15-value-overflow-incident.md` と統一。表記揺れ 9 ファイルを是正 (2026-07-24) |
...
| 並列項目の列挙 | `、` |
| 思考の終端 | `。` |
| 節と節を橋渡しする補足 | `。` で分ける、または接続詞 (`つまり` / `すなわち` / `ただし`) |
| 独立して立つほど区別される項目 | 改行または箇条書き |

一括置換は禁止 — 各出現を文脈で判断する。コードブロック・冒頭メタデータ・URL・引用された英語テキストの中の半角 `;` は対象外 (元の言語ではそれが許される)。

`npm run check:ja-glossary` (規則: `；` → `、 / 。 / 接続詞 / 改行`) により強制される。

#### ダッシュ類

**日本語の地の文では、節を橋渡しする補足にダッシュを使わない。** 読点・句点・接続詞・語順で書く。

**理由 (組版):** 和文の本文サイズでは、単体のダッシュ `—` が漢数字の `一`・長音符の `ー`・水平バーの `―` と見分けにくい。実フォント (Inter + JP フォールバック) で並べて描画し、通常の本文サイズで見分けにくいことを確認している。和文の伝統的な地の文が単体ダッシュではなく二倍ダーシ (`――`) を用いてきた背景にも、この混同を避ける意図が含まれると考えられる。

**とくに避ける形:**

| 形 | なぜ | 代わりに |
|---|---|---|
| `A — B — は C` (挟み込み) | 主語と述語の間に長い挿入が入り、読み下せなくなる。英語の同格構文をそのまま写した形で、和文に対応する用法がない | 句点で分けて言い切る |
| `X — 説明。` (文末の接着) | 限定を継ぎ足すために使われ、一文が伸びる | 句点で分ける / 「すなわち」「つまり」/ 格助詞で受ける |
| 1 段落に複数回 | 同じ構文の反復で単調になる | 上記のいずれか |

**例外 (ダッシュを使ってよい場所):**

- **タイトル・見出しの「本題 — 副題」区切り** (`Title Policy` の型。検索結果でも一般的な区切り慣行)
- **表のセル・定義リストの見出し区切り** (`項目 — 説明` の構造的用途)
...
|---|---|
| `―` 水平バー (U+2015) | ❌ エムダッシュへ |
| `─` 罫線 (U+2500、box drawing) | ❌ 散文中は常に誤り (字化け。ただしコードブロック内の `├──`/`└──` 等の ASCII アートは対象外) |
| `–` エンダッシュ (U+2013) | 日付・数値の範囲 (`2020–2024`) のみ |
| `-` ハイフン (半角) | 複合語・ハイフネーションのみ |

**一括置換は禁止。** 各出現を文脈で判断する (§ 半角スペースの規約と同じ理由)。地の文からダッシュを外すときは、記号を別の記号に置き換えるのではなく、**文の構造を組み直す**。
...
#### 対象外: 引用ブロック (`>` 始まりの行)・コードブロック・コード span

本規約の単独適用対象外は次の 3 つ:

- `>` で始まる引用ブロック行 (一次資料の verbatim 保持区間)
- ` ``` ` フェンスで囲まれたコードブロック (Mermaid タイムラインの意図的な空白も含む)
- ` ` `...` ` ` インライン code span (識別子・リテラル・verbatim 語の参照)
...
インライン `「...」` (日本語散文中の鍵括弧) は **散文の一部** として通常通り本規約の対象になる。`「」` 自体は免除マーカーではなく、内部の JA × ASCII 境界には半角スペースが要求される。

```
✅ 「Hashcash 仮説」      (内部に正規スペース)
✅ 「v0.1 リリース」      (内部に正規スペース)
✅ 「少しまちなー」       (全 JA、境界なし)
✅ 「Bitcoin」            (全 ASCII、境界なし)
❌ 「v0.1リリース」       (内部境界に空白なし → 検査違反)
❌ 「2chネラー」          (内部境界に空白なし → 検査違反)
```

通常の引用 (「○○ さんによると」 / 「『Winny』 を公開」 等) は検査と verbatim 保持の両方を満たすので、特殊措置は不要。「」 自体は JA 散文に必須かつ正当な punctuation。

#### Verbatim 合成語との衝突 (限定ケース)

引用したい語が**ASCII↔JA 境界を内部に持つ verbatim な一語**である場合 (= 空白を入れると意味が変わる固有合成語: 2 ちゃんねる固有語 `2chネラー`、誤記載した版数 `v0.1リリース` の原文保持等)、インライン `「」` + 空白挿入は **verbatim 性を破壊する** ので避ける。この稀ケースの対処:

| 手段 | 適用条件 |
|---|---|
| `>` blockquote として外出し | 引用が独立段落として成立する場合の正規手段 |
| ` ` `語` ` ` code span | 散文内で語を**識別子的に**参照する場合 (「`2chネラー` という語が示すのは」) |
| インライン `「」` + 正規空白 | verbatim 性を犠牲にしてもよい場合 (= verbatim でない言い回しなら OK) |

インライン `「」` 内で verbatim 強行 (空白なしで意味維持) は build 失敗、取りえない選択。
...
Mermaid はラベルを暗黙にしか改行しない。日本語には単語間スペースがないため、列幅より長い切れ目のない日本語の塊はレンダリング時にボックスからはみ出す。本番ではみ出した実例:
...
**Mermaid は図種別ごとに改行の挙動が異なる**。 日本語ラベルでは、 図種別に応じて改行手段を選ぶ:
...
```text
✅  サトシ最有力候補と<br/>名指される
❌  サトシ最有力候補と 名指される        (空白が表示に残る)
❌  サトシ最有力候補と名指される          (改行点なし、はみ出す)
```

timeline イベントラベルの例 (Satoshi biography から):

```text
✅  アダム・バックへの 最初の既知メール - 電子キャッシュ システムの相談 (8月)
❌  アダム・バックへの<br/>最初の既知メール - 電子キャッシュ<br/>システムの相談 (8月) (timeline は <br/> を honor しない)
```

`・` (中黒) や `、` はいずれの図種別でも改行点に**ならない** — 文章上の区切りには見えても Mermaid は改行点として扱わない。 改行が必要なら ASCII スペース (timeline 限定) か `<br/>` (それ以外) を意味的な区切り点に明示的に挿入する。

しきい値: CJK を含むトークンが**全角 12 文字**を超えると検出対象になる。 規則は flowchart / graph / classDiagram / stateDiagram / timeline / gantt 等のラベル系図に適用される。 タイトル系の対象外行:
- `title` キーワード行 (タイトルは別の、より幅の広いレイアウトを用いる)
- `subgraph ID[<text>]` のタイトル行 (subgraph ボックス全幅で描画され、 ボックスが内容に合わせて自動拡張するため列幅オーバーフローの前提が成立しない)
- `sequenceDiagram` のメッセージ行 (矢印に沿って水平自動拡張するため列幅オーバーフローの前提が成立しない)

**固有名の内部では分割しない**。改行挿入位置は固有名 (人名・チェーン名・引用タイトル等) の**外側**を選ぶ。固有名内に改行を入れると視覚的な断絶 (例: タイトル「ビットコイン実験の終結」 → 「ビットコイン 実験の終結」 で固有名が割れる) を生む。固有名そのものが span を 12 全角超に押し上げている場合は、ASCII 略号 (`BCH`、`BSV` 等) で span を断ち切るか、言い換えで長い span 自体を回避する:

```text
✗  ビットコイン 実験の終結              (タイトル内分割、 timeline)
✗  ビットコイン<br/>実験の終結          (タイトル内分割、 flowchart)
✗  ビットコインキャッシュ ハードフォーク (固有名と一般語の境界、固有名直後で切れて視覚的に不自然)

✓  ハーンが ビットコイン離脱を表明      (タイトルを平叙に言い換え、 timeline)
✓  ハーンが<br/>ビットコイン離脱を表明  (flowchart)
✓  BCH 分岐                              (略号 + 一般語、自然な境界)
...
**例外 — 行折返し由来の日本語文字間の空白は恒久的に対象外 (2026-07-11 確定):** 一次資料の書簡類の日本語本文は、元メールの桁揃えを模して固定幅で手折返しされており、折返し点は文字列上「日本語文字と日本語文字の間の改行・半角空白」として現れる。これはメール時代の横幅合わせの痕跡であって語の差ではなく、本サイトのレスポンシブな幅設計とも噛み合わない。描画上も現行ブラウザーは日本語文字間の改行を幅ゼロに詰めるため、読者には見えない。よって日本語文字同士に挟まれた空白・改行位置の差は cross-entry 整合の対象から恒久的に除外し、`check-quote-translation-consistency` も比較の両形 (語形キー・表記キー) でこの軸を畳み込む。折返しをほどいて 1 行化する清掃も行わない (差分汚染に見合う読者価値がない)。
