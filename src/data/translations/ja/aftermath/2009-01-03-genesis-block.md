...
![ジェネシスブロックを表す光るシアン色のブロックに、銀行救済を報じる新聞の一面が重なり、その下の施錠されたコインが使用不能な 50 BTC を示す。左側には消えかけた匿名化の痕跡アイコン、右側には後続のブロックとコインのアイコンが奥へ続く。](/BitcoinArchive/images/analysis/2009-01-03-genesis-block-hero.png)

ビットコインの最初のブロックに刻まれていたのは、バージョン文字列でもサトシ・ナカモトの名前でもなく、新聞の見出しだった。2009 年 1 月 3 日、サトシはそのブロックを v0.1 ソースにハードコードし、コインベーストランザクションへその日のタイムズ紙一面の見出しを転記した：
...
これがサトシが Bitcoin の設計の中に置いた、ただ一つの個人の声だ。他のあらゆる場面で、彼は身元の手がかりを丹念に削っていた。Tor を使い、メールアカウントをアドレスごとに使い分け、英米綴りを混在させ、タイピング癖に注意を払い、自ら退場した ([匿名性アーキテクチャ分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-anonymity-architecture/)に全体像)。設計の中のある一点だけ、彼は痕跡を削ることを止めて、逆に一つ挿入した。その一点に選んだ場所が、Bitcoin の最初のブロックだった。システムが持つもっとも永続的な表面である。
