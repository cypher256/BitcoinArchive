...
公開記録には、サトシがビットコインを作った *理由* についての自己言及がほとんど含まれていない。最も近いのは P2P Foundation 紹介での制度的信頼に対する対立的な位置づけ（「中央銀行は通貨を毀損しないと信頼されなければならない」）と、特定の設計トレードオフについての BitcoinTalk での散発的な言及である。本人によるより広いマニフェストは存在しない。
