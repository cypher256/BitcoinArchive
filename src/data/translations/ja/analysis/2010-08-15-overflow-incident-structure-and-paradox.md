...
ブロック 74638 で問題になったのは、1,840 億 BTC が生成されたことだけではない。旧ルールのノードはこのブロックを受け入れ、パッチ適用ノードは拒絶した。その結果、ネットワークは一時的に 2 つのチェーンを追うことになった。[バリュー・オーバーフロー事件の事後記事エントリー](/BitcoinArchive/ja/entries/aftermath/2010-08-15-value-overflow-incident/)が記録するのは、発見時刻、パッチの内容、チェーン再編成までにかかった時間である。ここでは同じ事件を、救出のなかで実際に何が可能だったのか、その制約がビットコイン黎明期の開発体制について何を露呈したのか、そしてトランザクションの形状から何が読めるのかという順に追う。
