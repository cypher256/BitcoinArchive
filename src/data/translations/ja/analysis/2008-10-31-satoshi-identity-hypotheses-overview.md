...
**経歴。** イギリスの暗号学者（1970 年生まれ）、エクセター大学で計算機科学博士号、*Hashcash*（1997 年）の考案者で、Hashcash はビットコインのホワイトペーパーで主要な先行事例として引用されたプルーフ・オブ・ワーク関数。Blockstream の共同創業者・CEO（2014 年）。
...
**反証。** 公開前通信（2008 年 8 月 20〜22 日）そのものが最強の反証となる：バックはサトシに Hashcash の引用について質問され、ウェイ・ダイの b-money を紹介した（サトシはそれまで知らなかった）— 第三者の応答であり、同一人物の自演ではない。バックは公的にサトシを別人として扱っており、最も顕著なのは [2024 年 2 月の COPA 対ライト裁判での証言](/BitcoinArchive/ja/entries/aftermath/2024-02-21-adam-back-retrospective-testimony/) で、サトシとの完全な通信記録を証人証拠として提出している。

**外部的状況。** 自己否定（最も顕著なのは COPA 対ライト裁判でのサトシ通信提出）／公開前通信は第三者応答として読める。
