---
title: "Re: Proof-of-Work難易度の上昇"
date: 2010-07-16T18:43:51.000Z
source: bitcointalk
sourceUrl: "https://bitcointalk.org/index.php?topic=43.msg3565#msg3565"
author: "Satoshi Nakamoto"
participants:
  - name: "Satoshi Nakamoto"
    slug: "satoshi-nakamoto"
description: "スレッド「Proof-of-Work難易度の上昇」におけるサトシ・ナカモトの返信。"
isSatoshi: true
secondarySources:
  - name: "Satoshi Nakamoto Institute"
    url: "https://satoshi.nakamotoinstitute.org/posts/bitcointalk/225/"
threadId: "bt-proof-of-work-difficulty-increasing"
threadPosition: 14
translationStatus: complete
---

その通りです。難易度調整は、ネットワーク全体で平均して1時間あたり6ブロックを生成するように維持しようとしています。ブロックが成熟するまでの時間は常に約20時間です。

最近の調整により、再び1時間あたり約6ブロックに近づきました。

ブロック間の時間を確認できるサイトがあり、ブロック68545以降は1ブロックあたり約10分に近くなっています:
[http://nullvoid.org/bitcoin/statistix.php](http://nullvoid.org/bitcoin/statistix.php)
