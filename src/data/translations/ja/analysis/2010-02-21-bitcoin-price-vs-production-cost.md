...
secondarySources:
  ...
  - name: "Alfred Marshall — Principles of Economics (1890), Book V, Chapter III (全文)"
    ...
    note: "「ハサミ」の一節。価値を支配するのが効用か生産費かを論じるのは、紙を切るのが上の刃か下の刃かを論じるようなもの — 短期は需要、長期は生産費が支配する。"
  ...
  - name: "Kristoufek — 「Bitcoin and its mining on the equilibrium path」 (Energy Economics, 2020)"
    ...
    note: "共通の長期均衡に向かうのは価格とマイニングコストの両方だが、調整するのはコストの側 — 数か月から 1 年かけて価格に合わせる。逆方向は無いか弱い。"
  ...
  - name: "CoinDesk — 「Bitcoin mining difficulty drops by most since 2021 as miners capitulate」 (2026 年 2 月 9 日)"
    ...
    note: "価格がモデル上のコストを下回り続けたときに折れたのはコストの側 — マイナーの撤退と難易度の低下。"
...
```mermaid
flowchart LR
    PRICE["市場価格"] -->|"価格がコストを上回る"| IN["ハッシュレート参入"]
    PRICE -->|"価格がコストを下回る"| OUT["ハッシュレート撤退"]
    IN --> DIFF["難易度が再調整<br/>（2,016 ブロックごと）"]
    OUT --> DIFF
    DIFF --> COST["1 コインあたりのコストが<br/>価格に近づく"]
    COST -.->|"供給は応答しない —<br/>発行量は固定"| PRICE
```
...
*[編者注：§5 の 2026 年の数値は動いている市場のスナップショットであり、仕組みの検証例として記録した。個別のドル水準そのものに意味はなく、§3 の構造的な議論はこれらの数値に依存しない。]*
