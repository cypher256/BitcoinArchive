...
16 年間の公開された追跡試行。数百件のフォーラム投稿、複数のメーリングリスト・アーカイブ、メール群、ソースコード、1000 ページ超のジャーナリズム —— すべて同じ問いに向けられた。そして精査に耐える特定は一つもない。本エントリは公開記録上の 3 つの観察可能な事実 —— (a) サトシ・ナカモトを特定しようとする公開された試行の反復、(b) サトシが残した発信材料の総量、(c) 続く不明状態 —— の間の関係を構造的な非対称性として読む。ここで解決すべき謎としてではなく、棚卸しすべき観察可能事実として扱う。本エントリは新しい身元候補を提案せず、既存候補を排除せず、公開外の追跡を主張しない。推論的な読み方は §6（限界）に限定する。
...
- **結論しない**。[身元仮説総覧 §5「Limits」](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identity-hypotheses-overview/) と[匿名性のアーキテクチャ §9「本エントリの限界」](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-anonymity-architecture/) と同じスタンスで、本エントリは観察と提示された代替案を並べるに留め、いずれかにコミットしない。

本同定非対称性分析は [2014 年 3 月 7 日サトシ P2P Foundation 復活エントリ](/BitcoinArchive/ja/entries/aftermath/2014-03-07-satoshi-p2p-foundation-return/)から参照される。同エントリは本非対称性枠組み ―「X はサトシである」という主張が「X はサトシではない」という主張より実質的に強い証拠を要する ― を、唯一の「私はドリアン・ナカモトではない」投稿を読むための分析的視座として用いる。
