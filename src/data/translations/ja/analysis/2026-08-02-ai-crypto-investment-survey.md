...
sourceNote: "本エントリーは既存文書の転載ではなく、ビットコイン・インスティテュート自身が行った一次的な実験である。2026年8月2日、5 つの AI 製品に同一の質問を投げかけ、その回答を記録した（1 件のみ例外があり、§6 に記す）。5 製品すべてのページと、各 AI の主張を検証した際の出典は secondarySources に列挙している。"
...
- **モデルのバージョンは記録しているが、本文には表示していない。** 各モデルは特定のバージョン・推論強度設定で実行した（第 2 節冒頭の非表示コメントを参照）。バージョン表記は数か月で古びて見えるため本文からは外したが、正確な設定を知りたい読者のためにページのソースには残している。
