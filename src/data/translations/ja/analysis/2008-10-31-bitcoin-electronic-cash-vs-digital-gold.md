...
ホワイトペーパーには、互いに引っ張り合うように見える二つの像がある。[タイトル](/BitcoinArchive/ja/entries/emails/cryptography/2008-10-31-bitcoin-whitepaper-final/)は電子キャッシュを掲げ、第 6 節は新しいコインを流通へ加える行為を金の採掘になぞらえる。この緊張は、二つの記述が同じ軸を指しているのかを確かめると見え方が変わる。ここでは、その違いをホワイトペーパーから最初に記録された購入へ、さらに希少性とスケーリングの制約が設計をデジタルゴールドへ押した経緯へ追う。
...
この設計から見えてくる論点は、「現金か金か」という二択ではない。ビットコインを持続させた同じ希少性が、なお使う余地を残せるのかという問題だ。設計はホワイトペーパーに書かれているが、どちらの顔が残るかは、いまネットワークがどう使われているかで決まる。
