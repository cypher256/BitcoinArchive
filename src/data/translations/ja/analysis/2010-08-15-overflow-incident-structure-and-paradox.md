...
1,840 億 BTC が一つのブロックで生成された。この数字がまず目を引く。だが、事件の核心はそれだけではない。[バリュー・オーバーフロー事件の事後記事エントリー](/BitcoinArchive/ja/entries/aftermath/2010-08-15-value-overflow-incident/)は、発見時刻、パッチの内容、チェーン再編成までにかかった時間を記録している。旧ルールのノードはブロックを受け入れ、パッチ適用ノードは拒絶した。一時的に、ビットコインは歴史の正統性をめぐって 2 つのチェーンを追うことになった。救出は成功した。だが同時に、その速さを可能にした中央集権的なリリース経路も露呈した。この成功と代償を、救出の仕組み、黎明期の開発体制、トランザクションの形状から読む。
...
この事件でいちばん読み違えやすいのは、救出の成功だけを祝うことだ。ライブな合意障害からネットワークが復旧できることは示された。同時に、その復旧は、システムが本来必要としないはずのリリース権限に依存した。事件を「ビットコインは生き残った」か「中央集権だった」かのどちらかに縮める気にはならない。記録に残っているのは、その二つが同時に起きたという、居心地の悪い事実だ。
