...
| 台帳 | 独自のブロックチェーン | およそ 24 のチェーン上で発行されるトークン |
...
USDC の供給は、マイニングでもステーキングでもなく、銀行送金で増減する。だがこの窓口は万人に同じ形で開かれているわけではない。承認された企業や機関は、Circle の口座 (Circle Mint) に米ドルを送金すると、スプレッドも取引手数料もなしに額面どおりの USDC を新規に受け取れる。これがミントである。ただしこの窓口を使うには最低 10 万ドルの預け入れが要り、それに満たない個人の買い手は取引所や OTC デスクへ回され、各社がその日ごとに設定するスプレッドを負担することになる。逆に USDC を米ドルへ戻したい機関は、Circle Mint の償還アドレスへ USDC を送る。Circle のスマートコントラクトがその USDC を焼却し (バーンし)、流通量から永久に差し引いたうえで、対応する米ドルを機関の銀行口座へ送金する。

摩擦のない直接の「1 対 1」を得られるのは Circle Mint にアクセスできる機関だけであり、個人の買い手が手にするのは、マーケットメーカーの提示価格を介した、もう一つの「1 対 1」である。この窓口で額面どおり無償にミントと償還を行えること自体が、USDC を 1 ドルへ引き戻す裁定の仕組みそのものであり、市場価格が 1 ドルからわずかでもずれれば、そこで利ざやを抜こうとする者が現れ、価格を押し戻す。
...
USDC は単一のチェーン上のトークンでもない。Circle 自身が開発した Cross-Chain Transfer Protocol (CCTP) は、いまではおよそ 24 のチェーンを結んでいる。仕組みは単純だ。送り手が送出元チェーンのスマートコントラクトに USDC を預けると、そのコントラクトは USDC を焼却し、宛先チェーンと受取人を記したメッセージを発行する。Circle の署名サービスがその焼却を確認し、送出元チェーンの確定を待ったうえで、宛先チェーン向けの承認署名を発行する。誰でもその署名を宛先チェーンのコントラクトへ提出でき、提出されると同額の USDC がそのチェーン上に新規で鋳造される。ラップドトークンも、資産を凍結するブリッジの金庫も存在しない。焼却と鋳造をつなぐのは Circle の署名だけだ。ビットコインのブロックチェーンには、そもそも複数チェーンをまたぐ仕組みが要らない。USDC のクロスチェーン転送を支えているのは、ビットコインが要らないと証明したまさにその一点、信頼できる第三者である。
