...
ウォレットはビットコインにおいて秘密鍵に触れる唯一のサブシステムである。他のコンポーネント（検証、中継、ストレージ、合意形成）はすべて公開データのみで動作する。この非対称性が、ウォレットをノードバイナリー内の密結合モジュールから論理的に（そして次第に物理的にも）分離されたプロセスへと進化させる原動力となった。
...
v0.1 ではウォレットはインターフェース境界なしにノードバイナリーに組み込まれていた。GUI、鍵ストア、マイナー、検証エンジンがすべて単一のプロセスと単一の Berkeley DB ファイル（`wallet.dat`）を共有していた。初回リリース時には RPC サーバーは存在しなかった。JSON-RPC が加わったのは発売から約 13 か月後、[v0.2.6（2010 年 2 月）](/BitcoinArchive/ja/entries/forum/bitcointalk/topic-63/2010-02-23-command-line-and-json-rpc/)のことで、その年の 7 月の [v0.3 リリース](/BitcoinArchive/ja/entries/aftermath/2010-07-06-bitcoin-v03-released/)で初めて主要機能に格上げされた。
...
サトシのウォレットは当初、鍵を独立に生成していた: 各秘密鍵は `wallet.dat` に保存される新規乱数で、先読みはなく、最後のバックアップ以降に作成された鍵は、ウォレットファイルが破壊されると失われた。この欠落は [v0.3.12（2010 年 9 月）](/BitcoinArchive/ja/entries/aftermath/2010-09-07-bitcoin-v0312-released/)まで続いており、同リリースが持つバックアップ手段は手動でファイルをコピーする新しい `backupwallet` RPC コマンドのみだった。サトシが実際にこの欠落を解消したのは翌月の v0.3.14（2010 年 10 月）で、100 鍵の事前生成プール（「鍵プール」）により、今日取得したバックアップが近い将来に生成されるアドレスもカバーできるようになった。この欠落は理論上の懸念にとどまらなかった。実際、[ダスティン・トランメルは 2009 年 1 月の時点で、まさにこのリスクをサトシに指摘していた](/BitcoinArchive/ja/entries/aftermath/2009-01-15-trammell-to-satoshi-address-verification/)。鍵プールがその欠落を解消する 2 年近く前のことである。
...
ビットコインアドレスはロックスクリプトのペイロードをユーザー向けにエンコードしたものである。ウォレットはアクティブな記述子からアドレスを生成し、アドレスが払い出されるたびに鍵プールを循環させる。具体的には、BIP 32 子鍵導出により次の未使用インデックスで新しい鍵を導出し、得られた鍵素材(Taproot ではさらに調整を加える)を以下のようにエンコードする。
...
無駄指標は、選択されたコインを今使う場合のコストと、仮想的な長期手数料率で使う場合のコストを比較して各候補選択を採点する。不必要なお釣り出力にはペナルティを与え、完全一致（お釣りなし）の解には報酬を与える。計算式は 3 つの要素のバランスをとる:
...
無駄スコアが低いほど良い。ウォレットは異なる戦略からのすべての候補選択を評価し、最も低い無駄の選択を採用する。

## 5. PSBT ワークフロー

部分署名ビットコイントランザクション（BIP 174 / BIP 370）はトランザクション構築と署名を分離する。PSBT は UTXO データ、導出パス、償還スクリプトなど署名者が必要とするすべてのメタデータを持ち運び、ブロックチェーンへのアクセスがないデバイスでも署名できるようにする。

<!-- visual: psbt-relay -->

ハードウェアデバイス（署名者 2）は署名する前に自身の画面で出力を検証する。完了者が最終的な scriptSig/witness を組み立てた後、抽出者がノードの検証・中継に使える完成したトランザクションを取り出す。
...
**分離が重要な理由。** ウォレットが扱う秘密鍵は、システム内で最も機密性の高いデータである。論理的分離（既に達成済み）と最終的な物理的プロセス分離により、鍵を保持するコンポーネントを制限された環境で動作させ、ノードは公開的に検証可能なチェーンデータを処理する。
