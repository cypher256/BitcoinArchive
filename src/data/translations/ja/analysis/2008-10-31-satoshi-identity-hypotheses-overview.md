...
**反証。** サッサマンの 2007〜2008 年の公的活動は [Wikipedia 項目](https://en.wikipedia.org/wiki/Len_Sassaman) および学会記録で確認できる — 2007 年 12 月に 24C3（Berlin）で *Anonymity for 2015*、Black Hat USA 2007（Las Vegas）で *Anonymity and its Discontents* を発表、2008 年 5 月に Bart Preneel と共著で *The Byzantine Postman Problem* を発表 — ただし全て匿名性ネットワーク専門領域で、デジタルキャッシュではない（ビットコイン設計空間との専門領域の分離）。Mixmaster（匿名性ルーティング）とビットコイン（デジタルキャッシュ）は暗号プリミティブで重なるが、アーキテクチャは異なる。サッサマンの 2008〜2011 年の KU ルーヴェン博士課程の負荷は、18 か月の集中ビットコイン設計と矛盾する。サッサマンとサトシの間の直接の文書接続は表面化していない。パターソンの沈黙は情報だが多義的（「確認しない」 と「否定しない」 のどちらにも読める）。
...
*[アーカイブ注：本アーカイブにはルルーの dedicated エントリー（伝記・E4M 告知・刑事事件文書・Mastermind 関連）がまだ存在しない。以下の概略は外部ソース — 主にジャーナリスト Evan Ratliff の『The Mastermind』（Random House、2019 年）および Atavist の長編記事 — と公開記録に基づく。具体的な日付や記述はアーカイブ検証ではなく外部由来として扱われたい。]*
