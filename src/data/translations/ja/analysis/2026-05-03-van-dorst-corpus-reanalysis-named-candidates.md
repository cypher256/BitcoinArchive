...
timeline
    ...
    2026 : カレイロウとカフィエロ NYT - 12 候補でアダム・バック首位、不確定
```
...
含意は方法論的に重要：**Burrows' Delta 指標は、本コーパスに対して追加の絞り込みなしに適用すると、最上位で信頼できない順位を生む**。絞り込みなしの比較の「勝者」 はコーパスの構造的副産物（短文アカウント、リメイラー転送テキスト、言語不一致の EC サイトテキスト）に支配されており、サトシへの本物の著者類似性ではない。より小規模なプール研究（Skye Grey、アストン、カフィエロ）はこの問題を、主題的根拠に基づいて選ばれた候補集合に事前絞り込みすることでのみ回避している — そしてこの絞り込み自体が、デイヴィッド・ジェラード、カフィエロ、その他批評者が指摘してきた主題による交絡問題を導入する。
