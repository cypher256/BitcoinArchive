...
合意形成の用語で言えば、`ConnectBlock()` が両方の財源を合算して `blockReward = nFees + GetBlockSubsidy(...)` を求め、コインベースの実際の出力額がこの合計を上回った場合にのみブロックを拒否する。新規発行分と手数料の各成分を個別の項目として検査することは一度もない。
...
実際には、ブロックテンプレートは手数料率順に上位から 4 MWU のウェイト上限まで埋まる。あるトランザクションがどの区分に落ち着くかは、その瞬間に待っているより高い手数料率のトランザクションが何件あるかだけで決まる。「次のブロック」と「待ち」を分ける固定の手数料率のしきい値は存在しない。
...
これは一回限りの証明ではなくフィードバックループだ。各段階は直前の段階に依存し、どの地点からでも強化されうるが、どの地点からでも弱まりうる。それはまさに、[§ 6](#6-手数料のみの将来) で検討する、輪の中の新規発行分という環がゼロへ近づくにつれて生じる懸念そのものだ。
...
この議論は、新規発行分が報酬の大部分を占めている間は最も強力である。半減によって新規発行分が減少するにつれ、インセンティブモデルはトランザクション手数料への依存を強める。この移行は [§ 6](#6-手数料のみの将来) および専用の[マイニング報酬枯渇分析](/BitcoinArchive/ja/entries/analysis/2026-05-18-mining-reward-exhaustion-fee-only-future/)で検討する。
