...
「メリークリスマス」 もなし。 「ありがとう」 もなし。 「君もね」 もなし。 返信は SPV クライアントモード設計から始まり、 8 段落の技術詳細を経て、 季節の挨拶に最後まで戻ってこなかった。 これは孤立した事例ではない。 約 2 年半 (2009 年 4 月から 2011 年 4 月) のおよぶ記録されたメール往復のなかで、 サトシは相手からの非技術的な発話に対し、 数えるほどの応答パターンを示す ― そのうち特定の 1 パターンからは、 ほぼ例外なく外れない。

本エントリーはその観察事例を整理し、 5 つのパターンに分類した上で、 ここに見える応答規律をサトシの匿名性アーキテクチャの **一層** として読み直す。 [サトシの匿名性アーキテクチャ](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-anonymity-architecture/)で扱う技術層 (Tor、 匿名化メール、 メタデータ削除) とは別の層である。 技術層は**経路**を不透明に保つ。 ここで扱う層は**内容**を不透明に保つ。

## 1. 観察事例の整理

以下の表は、 本アーカイブが一次資料で確認した事例を整理したものである。 網羅ではない ― マイク・ハーンの往復は 33 件、 マルッティ・マルミは 257 件、 ビットコイントークと暗号学メーリングリスト (`metzdowd.com`) を含めればさらに数百件の文脈が存在する。 ここでは「相手が明らかに非技術的な踏み込みをした」 場面を代表として選んでいる。
...
これら 5 つのパターンは表面の機構は異なるが、 構造的に共通する性質がある: **特定の個人、 特定の感情、 特定の予測は、 個別には決して扱われない**。 1 は機会そのものを拒む。 2 は話題を十分一般的な発言に洗い流して、 特定の個人への応答ではなくする。 3 は社交儀礼を最小限返して交流の場を破らない程度に保つ。 4 はサトシに関する外形的事実 (志向、 嗜好) を確認するが、 その理由を提供しない。 5 ― 例外 ― は領域 (反法定通貨という共有された立場) に限定されており、 そこでは個人情報を開示せずに人格性を表現できる。
...
| **コミュニケーション層 (本エントリー)** | 話題範囲の制限 (技術話のみ)、 最低限挨拶での受け流し、 一語での事実確認、 個人的嗜好・場所・文化・非技術話題への見解の議論拒否 | 内容水準での匿名解除 (**何が話されたか**を実在の人物に結びつける ― 文化・地理・年齢・政治的立場・感情的レジスター) |
...
2009 年 5 月 2 日のマルッティ・マルミとの初接触のやり取りは、 本アーカイブが浮かび上がらせた最も明確な例外である。 サトシはマルミに対し、 彼のビットコイン理解は「的を射ている」 と述べ、 `anti-state.com` フォーラムの反法定通貨の懐疑論者を「ネアンデルタール的」 と性格付け、 通貨採用について「燃えやすいが火花がない」 という比喩を共有する ― 個人情報の地盤ではなく **政治・文化的** 地盤に踏み込んでいる。
...
- **技術側での返礼が手厚い**。 サトシの技術返信は異例なほど長く、 細やかで、 助けになる ― 12 月 29 日にハーンが挨拶の代わりに受け取った SPV 説明は 8 段落にわたる設計議論である。 サトシが引き寄せた種類の通信相手にとって、 実質的な技術支援を受けることはクリスマスの挨拶を返してもらうより価値が高い。
...
- **受け手側の文体が時間を追って調整される**。 ハーンの 2011 年のメールには依然として個人的なひと言 (「お元気に」 「早く戻ってきて」) は含まれるが、 それを応答要求としては書かない。 受け手が無視してよい礼儀的な前置きとして添えている。 マルミの 2009 年 12 月の往復には祝祭日にもクリスマス挨拶が出てこない ― 最もシンプルな読みは、 マルミが既にこの通信路規範を内面化していたというものである。
...
- 約 2 年半 (2009 年 4 月から 2011 年 4 月) の記録されたメール往復のなかで、 サトシの非技術発話への応答は識別可能な 5 つのパターンに集約され、 そのうち 4 つは個人情報の通信路を閉じる方向に作用する。
- 2010 年 12 月 27 / 29 日のハーン ↔ サトシ往復 (「メリークリスマス、 サトシ」 → SPV だけの返信) は枠組み事例である ― 親しみのある季節の開きに対し 8 段落の設計議論が返り、 挨拶への承認は一度もない。
- 2009 年 1 月 10 日のハル・フィニーの「1 枚 1,000 万ドル / 1 億分の 1 の確率」 メールには個別返信がない ― サトシの同じスレッドの後の投稿は、 抽象的な「自己実現予言」 の発言を経由してのみその話題に近づき、 フィニーの具体予測には決して立ち入らない。
- 2009 年 5 月 2 日のマルミとの初接触は最も明確な例外だが、 そこでサトシが踏み込むのは個人情報の地盤ではなく **政治・文化的** 地盤であり、 実際に匿名解除につながる軸は規律のままに保たれている。
- この規律は、 [技術匿名性層](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-anonymity-architecture/) (Tor、 匿名化メール、 ヘッダー削除) と補完的な **コミュニケーション層の匿名性アーキテクチャ** として読まれるのが最もシンプルである。 技術層が**経路**を不透明に保つのに対し、 ここで扱う層は**内容**を不透明に保つ。
- パターンの数百回の機会にわたる一貫性が、 この読解を支える観察である ― 単一のメッセージでは読解は成立しない。

本非技術的スルー分析は[サイファーパンク独立到達分析](/BitcoinArchive/ja/entries/analysis/2008-10-31-cypherpunk-independent-arrival/)によって補完的読解として扱われる ― 独立到達エントリは記録されたサトシの実践の技術軸を扱い、 本エントリを「同じパターンを非技術的な開きに対する反応として記録した行動軸の対応物」 として参照する。

*[編者注：本エントリーは網羅的な列挙ではなく代表事例の整理である。 マルミ 257 件の全件走査とビットコイントーク記録の全件走査は将来の拡張作業として残されている。 浮かび上がる追加の例外事例は §1 のマトリクスに追記し、 §2 の 5 パターン分類に照らして評価することが望ましい。]*
