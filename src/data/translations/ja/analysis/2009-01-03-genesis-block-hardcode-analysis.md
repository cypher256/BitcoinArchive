...
6. 50 BTC の coinbase が使用不能なのは、Block 0 をトランザクションイベントではなくネットワークの初期状態として扱う設計の帰結として最もよく説明できる。これは §4 で論じた未帰属性と同じ bootstrap 設計から流れ出る。ひとつの設計判断、ふたつの効果

Block 0 は Bitcoin の中で最も象徴的なアーティファクトであり、同時に暗号学的な帰属が最も困難なブロックでもある。その困難さを生む bootstrap 構築と、50 BTC coinbase をトランザクションインデックスから除外する挙動は、いずれも `LoadBlockIndex()` の同一分岐に帰着する。その一つの設計判断が実装の便宜によるものか、創始の瞬間にも識別可能な痕跡を残さないという方針の一環であったかは、§4 が未決として残す解釈問題であり、ソースコード自身では答えられない唯一の問いである。
