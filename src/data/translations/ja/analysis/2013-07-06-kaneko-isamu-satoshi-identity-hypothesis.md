...
- **文章の文体**：サトシの英語は、学術メーリングリスト・カジュアルフォーラム・個人メール・リリースノートの 4 つの register を 32 か月にわたって切替えながら、ほぼネイティブの慣用句に到達し、日本語 substrate の痕跡（語順の不自然・冠詞や前置詞の non-native pattern・日本語固有概念の英語化痕）を一度も示さない。金子の記録された英語の学術文章は、能力はあるが同じ文体の幅は示さない。これは単独では決定的な不適格事由ではないが、ビットコインの記録に日本語系譜の痕跡が一切ないことと組み合わせると、事前確率を移動させる。
- **L1 register（47 氏発言からの一次資料 pin）**：金子の最も内側の文体は Winny 開発予告（2002-04-01、47 氏、[スレッドアーカイブ](https://winny.info/2ch/47.html)、[伝記の Winny 節に逐語](/BitcoinArchive/ja/participants/isamu-kaneko/)）に記録されている。`2chネラー` のような 2 ちゃんねるサブカル固有語彙、文末の「わ」「なー」 などの砕けた助詞、後に大規模化する P2P システムの開発予告にもかかわらず冒頭が「暇なんで」 で始まる自虐的軽さは、L1 日本語と特定文化圏への深い埋め込みを示す。末尾の決まり文句「少しまちなー」 は同スレッド以後の 47 氏ログで繰り返し現れる定型句で、register が一過性の演出ではなく自然発露であることを裏付ける。両 voice を整合させる仮説（金子が L1 = JP + L1 近似 = EN を 32 か月切替えなしで持続）は理論上可能だが、47 氏発言の自然な L1 性質は、金子側の L1 = 日本語深層を一次資料で固定する。
