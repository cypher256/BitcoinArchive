...
<!-- speaker: narrator -->
2014年10月1日、ピーター・トッドは BIP 65 を提案し、ビットコインのスクリプトシステムに新しいオペコード `OP_CHECKLOCKTIMEVERIFY` を導入した。この提案は既存の NOP2 オペコードを再定義し、指定されたブロック高またはタイムスタンプに達するまで使用できないタイムロック付きトランザクション出力を可能にした。
