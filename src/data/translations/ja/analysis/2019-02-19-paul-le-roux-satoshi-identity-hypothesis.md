...
ルルーの E4M（Encryption for the Masses、1999）は実在のオープンソースのディスク暗号化パッケージで、サイファーパンクメーリングリストで流通し、そのコードベースは 2002 年に TrueCrypt へフォークされた。これは暗号ソフトウェアを公開した記録であり、ビットコインの作者に必要とされる種類の能力を示す。

場も符合する。E4M の 1999 年の告知はサイファーパンクメーリングリストで行われ、そのリストの後継である [cryptography@metzdowd.com](/BitcoinArchive/ja/entries/analysis/2008-10-31-cypherpunk-independent-arrival/) こそ、サトシが 2008 年 10 月にビットコインのホワイトペーパーを投稿した場だ。同じ系統のメーリングリストが、9 年の間隔を挟んで 2 つの暗号ソフトウェアの発表を運んだことになる。
