...
候補集合の射程からの反論：ホワイトペーパーはウェイ・ダイの b-money も引用している。同じフォレンジック整合論によれば、ウェイ・ダイも並列の候補となる。この証拠は「サトシが明示的に引用したサイファーパンク」を候補集合として選ぶが、その中からバックを特定的に選び出すわけではない。ウェイ・ダイも同じ立場にある（候補全体の比較は[正体仮説の概観](/BitcoinArchive/ja/entries/analysis/2008-10-31-satoshi-identity-hypotheses-overview/)を参照）。
