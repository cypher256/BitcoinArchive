...
```mermaid
flowchart LR
  subgraph P2P[ピアツーピアの ビットコインネットワーク]
    N1((ノード))
    N2((ノード))
    N3((ノード))
    N4((ノード))
...
flowchart LR
  ...
  ADDR --> RECV[誰でも このアドレスに送れる]
  PK --> SIG[このアドレスに施錠された コインを <br/> 動かすトランザクションに 署名するのに使う]
```
...
sequenceDiagram
    ...
    M->>M: 待機中の トランザクションを集める
    M->>M: コインベース トランザクションを追加 (新規 BTC を発行)
    M->>M: ナンス 0 を試す ... ブロックハッシュ計算 ... 不一致
    M->>M: ナンス 1 を試す ... ブロックハッシュ計算 ... 不一致
    M->>M: ナンス 2 を試す ... ブロックハッシュ計算 ... 不一致
    M->>M: ... (数百万回試行) ...
    M->>M: ナンス 2083236893 を試す ... ハッシュ先頭ゼロが 規定数到達 ― 当たり
    M->>N: 新ブロックを ブロードキャスト
    N->>N: 全ノードが 検証して取り込む
```
...
```mermaid
flowchart TD
  CB[コインベース トランザクション] --> BR[ブロック報酬 <br/> 新規発行 BTC]
  TXS[ブロック内の 通常トランザクション] --> FEES[手数料合計]
...
```mermaid
flowchart TD
  REAL[本物のチェーン <br/> ... B98 - B99 - B100 - B101 - B102 - B103 ... <br/> 正直なマイナーが 延ばし続ける]
