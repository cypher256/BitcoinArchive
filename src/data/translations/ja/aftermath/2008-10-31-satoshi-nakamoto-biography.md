...
*異例の配布形式：*
Bitcoin v0.1は.rarアーカイブとして配布された。SourceForgeでは.zipや.tar.gzが標準であり、オープンソースプロジェクトで.rarが使われることは稀だった。.rarは非公式なソフトウェア配布チャネルでより一般的に使われていた形式である。この選択の理由は不明である。参考として、当時の非公式配布コミュニティで一般的だった慣習とサトシの配布手法を比較する：
