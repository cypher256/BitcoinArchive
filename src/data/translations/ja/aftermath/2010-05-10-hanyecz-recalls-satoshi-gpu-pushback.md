...
*[編者注：本エントリーは回想に基づく記事であり、メールの一次資料ではない。一次資料で検証可能なのは、ハニエツによる GPU マイニング発表（2010 年 5 月 10 日、Bitcointalk フォーラム公開記録）のみ。サトシ側のやり取り、すなわち本記事で引用されるメールは、ハニエツの後年のインタビューでの回想に依拠する。元のメール文言は公開されておらず、以下に掲載するのは CoinDesk（2025 年）、Bitcoin Magazine、Cointelegraph 等のインタビューでハニエツが思い出した語感と内容である。]*
