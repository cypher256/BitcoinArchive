...
<!-- speaker: narrator -->
トランメルは問題を正確に診断していた。すなわち、v0.1.0 の通信バグにより彼のノードはブロックをネットワークにブロードキャストできず、採掘したすべてのブロックが孤立していた([ブロック・チェーン設計のエントリー](/BitcoinArchive/ja/entries/design/2009-01-03-bitcoin-block-chain-design/)が、その背後にある仕組みと、孤児ブロック・失効ブロックという現代的な区別を詳述している)。サトシは次の返信でこれを確認した。
