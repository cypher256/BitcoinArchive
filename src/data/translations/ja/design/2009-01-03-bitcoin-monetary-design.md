...
マイナーは報酬の全額より**少なく**請求してもよい。未請求の手数料は永久に消滅する — 実効供給量が減少する。初期のブロック数件（サトシや初期マイナーがマイニングしたもの）は、偶然かまたは意図的に、コインベース出力が上限を下回っている。この仕組みは初期のユーザーを困惑させるほど不透明だった ―― [説明のつかない 50.44 BTC の報酬が 2010 年 2 月の BitcoinTalk フォーラム投稿を引き出し](/BitcoinArchive/ja/participants/michael-marquardt/)、これが新規発行分に手数料が加算される仕組みについての最も早い公開説明の一つとなった。
